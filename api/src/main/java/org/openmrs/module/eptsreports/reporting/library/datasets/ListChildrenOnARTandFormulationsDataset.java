...
public class ListChildrenOnARTandFormulationsDataset extends BaseDataSet {
  ...
  public DataSetDefinition constructDataset() {
    ...
    DataDefinition identifierDef = new ConvertedPatientDataDefinition("identifier",
        new PatientIdentifierDataDefinition(identifierType.getName(), identifierType), identifierFormatter);
    ...
    patientDataSetDefinition.addColumn("artstartdate", getArtStartDate(), "onOrBefore=${endDate},location=${location}",
    ...
    patientDataSetDefinition.addColumn("ontbtreatment", this.getPatientsActiveOnTB(),
        "endDate=${endDate},location=${location}", new EncounterDatetimeConverter());

    patientDataSetDefinition.addColumn("lastpickupdate", this.getLastDrugPickupDate(),
        "endDate=${endDate},location=${location}", new EncounterDatetimeConverter());

    patientDataSetDefinition.addColumn("lastregimewithdrawal", getLastARVRegimen(),
        "onOrBefore=${endDate},location=${location}", new EncounterDatetimeConverter());
    // Formulations
    patientDataSetDefinition.addColumn("formulation1", getFormulations(1), "onOrBefore=${endDate},location=${location}",
        new SimpleResultsConverter());

    patientDataSetDefinition.addColumn("formulation2", getFormulations(2), "onOrBefore=${endDate},location=${location}",
        new SimpleResultsConverter());

    patientDataSetDefinition.addColumn("formulation3", getFormulations(3), "onOrBefore=${endDate},location=${location}",
        new SimpleResultsConverter());

    patientDataSetDefinition.addColumn("formulation4", getFormulations(4), "onOrBefore=${endDate},location=${location}",
        new SimpleResultsConverter());

    patientDataSetDefinition.addColumn("nextpickupdate", getNextDrugPickupDate(),
        "onOrBefore=${endDate},location=${location}", new EncounterDatetimeConverter());

    patientDataSetDefinition.addColumn("lastconsultationdate", this.getLastFollowupConsultationDate(),
        "endDate=${endDate},location=${location}", new EncounterDatetimeConverter());

    patientDataSetDefinition.addColumn("lastregimeconsultation", this.getARVRegimenLastConsultationDate(),
        "endDate=${endDate},location=${location}", new EncounterDatetimeConverter());

    patientDataSetDefinition.addColumn("weight", this.getWeightLossLastConsultationDate(),
        "endDate=${endDate},location=${location}", new EncounterDatetimeConverter());

    patientDataSetDefinition.addColumn("familyapproachlastconsultation",
        this.getAbordagemFamiliarOnLastConsultationDate(), "endDate=${endDate},location=${location}",
        new EncounterDatetimeConverter());

    patientDataSetDefinition.addColumn("quartelydismissallastconsultation",
        this.get3MonthsDispensationOnLastConsultationDate(), "endDate=${endDate},location=${location}",
        new EncounterDatetimeConverter());

    patientDataSetDefinition.addColumn("nextconsultationdate", this.getNextFollowUpConsultationDate(),
        "endDate=${endDate},location=${location}", new EncounterDatetimeConverter());
  ...
  private DataDefinition getPatientsActiveOnTB() {
    ...
    String sql = " SELECT final_query.patient_id, CASE WHEN final_query.result_Value IS NOT NULL THEN 'ACTIVE' WHEN final_query.result_Value IS NULL THEN 'INACTIVE' "
        + " FROM " + "( " + "                SELECT p.patient_id, o.value_coded  AS result_Value"
        + "                FROM patient p "
        + "                         INNER JOIN encounter e on p.patient_id = e.patient_id "
        + "                         INNER JOIN obs o on e.encounter_id = o.encounter_id "
        + "                WHERE p.voided = 0 " + "                  AND e.voided = 0 "
        + "                  AND o.voided = 0 " + "                  AND e.location_id = :location "
        + "                  AND e.encounter_type = ${6} " + "                  AND ( "
        + "                        (o.concept_id = ${1268} AND o.value_coded = 1256) " + "                        OR "
        + "                        (o.concept_id = ${1113} AND o.obs_datetime "
        + "                            BETWEEN DATE_SUB(:endDate, INTERVAL 210 DAY) AND :endDate) "
        + "                    ) " + "                  AND e.encounter_datetime <= :endDate "
        + "                UNION  " + "                SELECT p.patient_id, o.value_datetime AS result_Value"
        + "                FROM patient p "
        + "                         INNER JOIN encounter e on p.patient_id = e.patient_id "
        + "                         INNER JOIN obs o on e.encounter_id = o.encounter_id "
        + "                WHERE p.voided = 0 " + "                  AND e.voided = 0 "
        + "                  AND o.voided = 0 " + "                  AND e.location_id = :location "
        + "                  AND e.encounter_type IN (${6},${9}) " + "                  AND o.concept_id = ${1113} "
        + "                  AND o.value_datetime "
        + "                            BETWEEN DATE_SUB( :endDate, INTERVAL 210 DAY ) AND :endDate "
        + "                  AND e.encounter_datetime <= :endDate " + "                UNION  "
        + "                SELECT p.patient_id, o.value_coded AS result_Value " + "                FROM patient p "
        + "                         INNER JOIN encounter e on p.patient_id = e.patient_id "
        + "                         INNER JOIN obs o on e.encounter_id = o.encounter_id "
        + "                WHERE p.voided = 0 " + "                  AND e.voided = 0 "
        + "                  AND o.voided = 0 " + "                  AND e.location_id = :location "
        + "                  AND e.encounter_type = ${53} " + "                  AND o.concept_id = ${1406} "
        + "                  AND o.value_coded = ${42} " + "                  AND o.obs_datetime "
        + "                    BETWEEN DATE_SUB ( :endDate, INTERVAL 210 DAY ) AND :endDate "
        + "                UNION  " + "                SELECT p.patient_id , cn.name AS result_Value "
        + "                FROM patient p "
        ...
        + "                WHERE p.voided = 0 " + "                  AND pp.voided = 0 "
        + "                  AND ps.voided = 0 " + "                  AND ps.state = ${6269} "
        + "                  AND pgr.program_id = ${5} " + "                  AND cn.locale = 'pt' "
        + "                  AND ps.start_date >= DATE_SUB(:endDate, INTERVAL 210 DAY) "
        + "                  AND ps.end_date <= :endDate " + "               UNION  "
        + "                SELECT p.patient_id, o.value_coded  AS result_Value" + "                FROM patient p "
        + "                         INNER JOIN encounter e on p.patient_id = e.patient_id "
        + "                         INNER JOIN obs o on e.encounter_id = o.encounter_id "
        + "                WHERE p.voided = 0 " + "                  AND e.voided = 0 "
        + "                  AND o.voided = 0 " + "                  AND e.encounter_type = ${6} "
        + "                  AND e.location_id = :location " + "                  AND o.concept_id = ${23761} "
        + "                  AND o.value_coded = ${1065} " + "                  AND e.encounter_datetime "
        + "                    BETWEEN DATE_SUB ( :endDate, INTERVAL 210 DAY ) AND :endDate" + ") AS final_query "
        + " WHERE p.voided = 0";
  ...
  private DataDefinition getLastARVRegimen() {
    ...
    String sql = " SELECT p.patient_id, o.value_coded " + " FROM patient p" + "   INNER JOIN "
        ...
        + "         AND e.encounter_datetime <= :endDate " + "         AND e.encounter_type = ${18}"
        + "     GROUP BY p.patient_id " + "   ) max_encounter ON p.patient_id=max_encounter.patient_id"
        + "     INNER JOIN encounter e ON p.patient_id= e.patient_id "
        + "     INNER JOIN obs ob ON e.encounter_id = o.encounter_id "
        + "     INNER JOIN concept_name cn on ob.concept_id=cn.concept_id " + " WHERE  p.voided = 0"
        + "     AND e.voided = 0 " + "     AND o.voided = 0" + "     AND cn.locale='pt'"
        + "     AND max_encounter.encounter_datetime = e.encounter_datetime " + "     AND e.encounter_type = ${18} "
        + "     AND e.location_id = :location " + "     AND o.concept_id = ${1088} "
        + "     AND o.value_coded IS NOT NULL " + "     AND e.encounter_datetime <= :endDate ";
  ...
  private DataDefinition getLastDrugPickupDate() {
    ...
    String sql = " SELECT p.patient_id, MAX(e.encounter_datetime) " + " FROM   patient p  "
        ...
        + "          AND e.encounter_type = ${18} " + "         AND e.encounter_datetime <= :endDate "
  ...
  private DataDefinition getNextDrugPickupDate() {
    ...
    String sql = "" + "  SELECT p.patient_id, o.value_datetime " + " FROM   patient p  "
        + "     INNER JOIN encounter e  " + "         ON p.patient_id = e.patient_id  " + "     INNER JOIN obs o  "
        + "         ON e.encounter_id = o.encounter_id  " + "     INNER JOIN ("
        + "             SELECT p.patient_id, MAX(e.encounter_datetime) as e_datetime "
        + "             FROM   patient p  " + "                 INNER JOIN encounter e  "
        + "                     ON p.patient_id = e.patient_id  " + "                 INNER JOIN obs o  "
        + "                     ON e.encounter_id = o.encounter_id  " + "             WHERE  p.voided = 0  "
        ...
        + "                   ) most_recent  ON p.patient_id = most_recent.patient_id   " + " WHERE  p.voided = 0  "
        + "     AND e.voided = 0  " + "     AND o.voided = 0  " + "     AND e.location_id = :location "
        + "     AND e.encounter_type IN (${18}) " + "     AND e.encounter_datetime <= :endDate "
        + "     AND e.encounter_datetime = most_recent.e_datetime " + "     AND o.concept_id = ${5096} ";
  ...
  private DataDefinition getLastFollowupConsultationDate() {
    ...
    String sql = " SELECT p.patient_id, MAX(e.encounter_datetime) " + " FROM   patient p  "
        ...
        + "         AND e.location_id = :location " + "         AND e.encounter_type IN (${6}, ${9}) "
        + "         AND e.encounter_datetime <= :endDate " + "GROUP BY p.patient_id ";
  ...
  private DataDefinition getARVRegimenLastConsultationDate() {
    ...
    String sql = "" + "  SELECT p.patient_id, o.value_coded " + " FROM   patient p  " + "     INNER JOIN encounter e  "
        + "         ON p.patient_id = e.patient_id  " + "     INNER JOIN obs o  "
        + "         ON e.encounter_id = o.encounter_id  " + "     INNER JOIN ("
        + "             SELECT p.patient_id, MAX(e.encounter_datetime) as e_datetime "
        + "             FROM   patient p  " + "                 INNER JOIN encounter e  "
        ...
        + "                   ) most_recent  ON p.patient_id = most_recent.patient_id   " + " WHERE  p.voided = 0  "
        + "     AND e.voided = 0  " + "     AND o.voided = 0  " + "     AND e.location_id = :location "
        + "     AND e.encounter_type IN (${6}, ${9}) " + "     AND e.encounter_datetime <= :endDate "
        + "     AND e.encounter_datetime = most_recent.e_datetime " + "     AND o.concept_id = ${1087} ";
    StringSubstitutor substitutor = new StringSubstitutor(valuesMap);
  ...
  private DataDefinition getWeightLossLastConsultationDate() {
    ...
    String sql = "" + "  SELECT p.patient_id, o.value_numeric " + " FROM   patient p  "
        + "     INNER JOIN encounter e  " + "         ON p.patient_id = e.patient_id  " + "     INNER JOIN obs o  "
        + "         ON e.encounter_id = o.encounter_id  " + "     INNER JOIN ("
        + "             SELECT p.patient_id, MAX(e.encounter_datetime) as e_datetime "
        + "             FROM   patient p  " + "                 INNER JOIN encounter e  "
        ...
        + "     AND e.voided = 0  " + "     AND o.voided = 0  " + "     AND e.location_id = :location "
        + "     AND e.encounter_type IN (${6}, ${9}) " + "     AND e.encounter_datetime <= :endDate "
        + "     AND e.encounter_datetime = most_recent.e_datetime " + "     AND o.concept_id = ${5089} ";
  ...
  private DataDefinition getAbordagemFamiliarOnLastConsultationDate() {
    ...
    String sql = "" + "  SELECT p.patient_id, o.value_coded " + " FROM   patient p  " + "     INNER JOIN encounter e  "
        ...
        + "     AND e.voided = 0  " + "     AND o.voided = 0  " + "     AND e.location_id = :location "
        + "     AND e.encounter_type IN (${6}, ${9}) " + "     AND e.encounter_datetime <= :endDate "
        + "     AND e.encounter_datetime = most_recent.e_datetime " + "     AND o.concept_id = ${23725} ";
  ...
  private DataDefinition get3MonthsDispensationOnLastConsultationDate() {
    ...
    String sql = ""
        ...
        + "     INNER JOIN obs o  " + "         ON e.encounter_id = o.encounter_id  " + "     INNER JOIN ("
        + "             SELECT p.patient_id, MAX(e.encounter_datetime) as e_datetime "
        + "             FROM   patient p  " + "                 INNER JOIN encounter e  "
        + "                     ON p.patient_id = e.patient_id  " + "                 INNER JOIN obs o  "
        + "                     ON e.encounter_id = o.encounter_id  " + "             WHERE  p.voided = 0  "
        ...
        + "     AND e.voided = 0  " + "     AND o.voided = 0  " + "     AND e.location_id = :location "
        + "     AND e.encounter_type IN (${6}, ${9}) " + "     AND e.encounter_datetime <= :endDate "
        + "     AND e.encounter_datetime = most_recent.e_datetime " + "     AND o.concept_id = ${23739} "
  ...
  private DataDefinition getNextFollowUpConsultationDate() {
    ...
    String sql = " SELECT p.patient_id, MAX(o.value_datetime) " + " FROM   patient p  "
        + "         INNER JOIN encounter e  " + "                         ON p.patient_id = e.patient_id  "
        + "         INNER JOIN obs o  " + "                         ON e.encounter_id = o.encounter_id  "
        + " WHERE  p.voided = 0  " + "         AND e.voided = 0  " + "         AND o.voided = 0  "
        + "         AND e.location_id = :location " + "         AND e.encounter_type IN (${6}, ${9}) "
        + "         AND o.concept_id = ${1410}  " + "         AND e.encounter_datetime <= :endDate "
        + " GROUP BY p.patient_id ";
...
private DataDefinition getFormulations(Integer column){
  ...
  CalculationDataDefinition calculationDataDefinition = Context.getRegisteredComponents(ListOfPatientsFormulationCalculation.class).get(0);
