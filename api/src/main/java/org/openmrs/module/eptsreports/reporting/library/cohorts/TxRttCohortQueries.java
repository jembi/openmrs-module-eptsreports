  ...
  public CohortDefinition getCd4Result(
    ...
    String query =
        "SELECT patient_id "
            ...
            + "               Min(Date(cd4.cd4_date)) cd4_date "
            ...
            + "                       INNER JOIN obs o "
            + "                               ON o.encounter_id = e.encounter_id "
            + "                       INNER JOIN (SELECT first.patient_id, "
            ...
            + "                                   GROUP  BY first.patient_id) returned "
            + "                               ON returned.patient_id = p.patient_id "
            + "                WHERE  p.voided = 0 "
            + "                       AND e.voided = 0 "
            + "                       AND o.voided = 0 "
            ...
            + "                           INTERVAL 30 day) "
            + "                       AND Date(e.encounter_datetime) <= "
            + "                           Date_add(returned.first_pickup, "
            + "                           INTERVAL 28 day) "
            + "                GROUP  BY p.patient_id "
            + "                UNION "
            + "                SELECT p.patient_id, "
            + "                       Date(o.obs_datetime) cd4_date "
            + "                FROM   patient p "
            + "                       INNER JOIN encounter e "
            + "                               ON e.patient_id = p.patient_id "
            + "                       INNER JOIN obs o "
            ...
            + "                           Date_add(returned.first_pickup, "
            + "                           INTERVAL 28 day)) cd4 "
            + "        GROUP  BY cd4.patient_id) oldest_cd4";
