...
public class ListOfPatientsWithMdsEvaluationQueries {
  ...
  public static String getCohort24To36MonthsPatients() {
    return "SELECT "
        + "    art_patient.patient_id, art_patient.first_pickup "
        + "FROM "
        + "    ( "
        + "        SELECT "
        + "            first.patient_id, "
        ...
        + "                  AND e.encounter_type = ${18} "
        + "                  AND e.location_id = : location "
        + "                  AND e.encounter_datetime <= CONCAT(: evaluationYear, '-06-20') "
        + "                GROUP BY "
        + "                    p.patient_id "
        + "                UNION "
        + "                SELECT "
        ...
        + "                  AND o.value_coded = ${1065} "
        + "                  AND o2.concept_id = ${23866} "
        + "                  AND o2.value_datetime <= CONCAT(: evaluationYear, '-06-20') "
        + "                GROUP BY "
        + "                    p.patient_id "
        + "            ) first "
        + "        GROUP BY "
        ...
        + "            CONCAT(: evaluationYear, '-12-21'), "
        + "            INTERVAL 3 YEAR "
        + "                                ) "
        + "  AND art_patient.first_pickup <= DATE_SUB( "
        + "        CONCAT(: evaluationYear, '-06-20'), "
        + "        INTERVAL 2 YEAR "
        + "                                  ) "
  ...
  public static String get36MonthsCohortPatients() {
    return "SELECT "
        + "    art_patient.patient_id, art_patient.first_pickup "
        + "FROM "
        + "    ( "
