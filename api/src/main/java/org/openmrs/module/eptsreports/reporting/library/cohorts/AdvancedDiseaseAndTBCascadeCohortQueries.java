  ...
  public CohortDefinition getPregnantWithoutCd4() {
    ...
    String query =
        "SELECT    min_pregnant.patient_id "
            ...
            + "                                       WHERE      p.voided = 0 "
            + "                                       AND        e.voided = 0 "
            + "                                       AND        pregnancy.voided = 0 AND e.encounter_type = ${53} "
            ...
            + "                                       UNION "
            + "                                       SELECT     p.patient_id, "
            + "                                                  MAX(date(o.value_datetime)) AS pregnancy_date "
            + "                                       FROM       patient p "
            + "                                       INNER JOIN person pe "
            + "                                       ON         p.patient_id = pe.person_id "
            + "                                       INNER JOIN encounter e "
            ...
            + "                     AND        e.location_id = :location "
            + "                     AND        DATE(e.encounter_datetime) > min_pregnant.pregnancy_date AND DATE(e.encounter_datetime) <= DATE_SUB(:endDate, INTERVAL 1 MONTH) "
            + "                     GROUP BY   e.patient_id "
            + "                     UNION "
            + "                     SELECT     e.patient_id "
            + "                     FROM       encounter e "
            ...
            + "                                           AND        o.value_coded IS NOT NULL ) ) "
            + "                     AND        e.location_id = :location "
            + "                     AND        o.obs_datetime > min_pregnant.pregnancy_date AND o.obs_datetime <= DATE_SUB(:endDate, INTERVAL 1 MONTH) "
            + "                     GROUP BY   e.patient_id ) ";
