  ...
  public DataDefinition pregnantBreasfeediDefinition() {
    ...
    String sql =
        "SELECT p.patient_id, CASE "
            ...
            + "                            AND e.encounter_type = ${5} AND e.encounter_datetime "
            ...
            + "                            AND e.encounter_type = ${5} AND o.value_datetime "
            ...
            + "                            WHERE p.voided=0 AND e.voided=0 AND o.voided=0 AND concept_id= ${1279} "
            + "                            AND   "
            + "                            e.encounter_type = ${5} AND e.encounter_datetime between DATE_SUB(curdate(), INTERVAL 9 MONTH) AND curdate() "
            + "                            AND e.location_id=:location  AND pe.gender='F' GROUP BY p.patient_id   "
            + "                            UNION   "
            + "                            Select p.patient_id,  MAX(e.encounter_datetime) as pregnancy_date   "
            + "                            FROM patient p   "
            ...
            + "                            INNER JOIN obs o ON e.encounter_id=o.encounter_id   "
            + "                            WHERE p.voided=0 AND e.voided=0 AND o.voided=0 AND concept_id=  ${1600} "
            + "                            AND  e.encounter_type = ${5} AND e.encounter_datetime between DATE_SUB(curdate(), INTERVAL 9 MONTH) AND curdate() "
            + "                            AND e.location_id=:location AND pe.gender='F' GROUP BY p.patient_id   "
            + "                            UNION   "
            + "                            Select p.patient_id, MAX(e.encounter_datetime) as pregnancy_date   "
            + "                            FROM patient p   "
