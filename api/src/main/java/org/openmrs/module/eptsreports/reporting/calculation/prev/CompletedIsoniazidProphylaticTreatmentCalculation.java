...
public class CompletedIsoniazidProphylaticTreatmentCalculation extends AbstractPatientCalculation {
    ...
    if (onOrAfter != null && onOrBefore != null) {
      ...
      CalculationResultMap startDrugsObservations =
          ePTSCalculationService.getObs(
              ...
              onOrAfter,
              context);
      // III. Patients who have Regime de TPT with the values marked on the first pick-up date during the previous period
      ...
      // VI is omitted, actually is: vii.have at least 173 days apart between the IPT Start date (the most oldest date from above criterias) and IPT End date (the most recent date from above criterias).
     ...
     // XIII
      ...
      //I
        ...
        Obs regimeTPT1stPickUpPreviousPeriod =
                EptsCalculationUtils.obsResultForPatient(regimeTPT1stPickUpPreviousPeriodMap, patientId);
        ...
        Date iptStartDate =
            getMinOrMaxObsDate(Arrays.asList(startProfilaxiaObs,
                    startDrugsObs,this.exclude(regimeTPT1stPickUpPreviousPeriod,exclusionOutrasPrescricoesPreviousPeriod),
                    this.exclude(outrasPrescricoesPreviousPeriod,exclusionOutrasPrescricoesPreviousPeriod)),
                    Priority.MIN);

        Date iptEndDate =
            getMinOrMaxObsDate(
                Arrays.asList(endProfilaxiaObs,endDrugsObs),
                Priority.MAX);
        //viii
        int yesAnswers =
        ...
        int filtOccurencies  = evaluateOccurrence(filts,iptStartDate,6,7);
        ...
        int dtINHOccurencies  = evaluateOccurrence(dtINHs,iptStartDate,2,5);
        ...
        int dfiltdtINHsOccurencies  = evaluateOccurrence(dfiltdtINHs,iptStartDate,2,5);
        ...
        int atLeast3FichaClínicaINHAOccurencies  = evaluateOccurrence(atLeast3FichaClínicaINHA,iptStartDate,3,7);
        ...
        int atLeast3FichaClínicaINHBOccurencies  = evaluateOccurrence(atLeast3FichaClínicaINHB,iptStartDate,1,7);
        ...
        int atLeast3FILTINHMensal1FILTDTINHAOccurencies  =
                evaluateOccurrence(atLeast3FILTINHMensal1FILTDTINHA,iptStartDate,1,7);
        ...
        int atLeast3FILTINHMensal1FILTDTINHBOccurencies  =
                evaluateOccurrence(atLeast3FILTINHMensal1FILTDTINHB,iptStartDate,3,7);
  ...
  private Date getMinOrMaxObsDate(
      List<Obs> obss, Priority priority) {
  ...
  private  Obs exclude(Obs firstINHDate, List<Obs> bGroup){
    Obs obs = firstINHDate;
    if(firstINHDate!=null) {

      for (Obs o : bGroup) {

        if (o.getEncounter().getEncounterDatetime().compareTo(DateUtils.addMonths(firstINHDate.getEncounter().getEncounterDatetime(), -7)) >= 0
                &&
                o.getEncounter().getEncounterDatetime().compareTo(firstINHDate.getEncounter().getEncounterDatetime()) < 0) {
          obs = null;
          break;
        }
      }
  ...
  private int evaluateOccurrence (List<Obs> obss, Date iptStartDate, int nTimes, int plusIPTDate){
    ...
    for(Obs o : obss){
      ...
      if(o.getEncounter().getEncounterDatetime().compareTo(DateUtils.addMonths(iptStartDate,plusIPTDate))>=0
        &&
