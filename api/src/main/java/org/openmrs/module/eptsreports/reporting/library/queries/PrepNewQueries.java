  ...
  public static String getPatientAgeBasedOnPrepStartDate(
    ...
    String query =
        "SELECT patient_id "
            ...
            + "		INNER JOIN ( "
            + "			SELECT patient_id, earliest_date_ever "
            + "			FROM   (SELECT p.patient_id, Min(tbl.earliest_date) AS 	earliest_date_ever "
            + "				FROM   patient p "
            ...
            + "				WHERE  clients.earliest_date_ever BETWEEN :startDate AND :endDate "
            + "		) clients "
            + "		ON pat.patient_id = clients.patient_id "
            + ") fin "
