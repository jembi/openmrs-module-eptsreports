  ...
  public CohortDefinition getPatientsTransferredOut() {
    ...
    String query =
        "SELECT   mostrecent.patient_id "
            ...
            + "                    ON         latest_fila.patient_id = latest.patient_id "
            + "                    INNER JOIN "
            + "                               ( "
            + "                                          SELECT     p.patient_id, "
            + "                                                     max(o.value_datetime) AS value_datetime "
            ...
            + "                                          AND        o.value_datetime BETWEEN :startDate AND        :endDate "
            + "                                          AND        e.location_id = :location "
            + "                                          GROUP BY   p.patient_id ) levantou_arv "
            + "                    ON         levantou_arv.patient_id = latest.patient_id "
            + "                    WHERE      last_date > date_add(latest_fila.value_datetime, interval 1 day) "
            + "                    AND        last_date <= date_add(levantou_arv.value_datetime, interval 31 day) "
            + "                    AND        latest.patient_id NOT IN "
            + "                               ( "
            + "                                          SELECT     p.patient_id "
            + "                                          FROM       patient p "
            + "                                          INNER JOIN encounter e "
            ...
            + "                                          AND        e.location_id = :location "
            + "                                          GROUP BY   p.patient_id ) "
            + "                    GROUP BY   latest.patient_id )mostrecent "
