...
@Component
public class ReturnedAdnITTDifferenceDateCalculation extends AbstractPatientCalculation {
  ...
  private final String LESS_THAN_365_DAYS = "lessThan365Days";
  private final String PREVIOUS_PERIOD = "previousPeriod";
  ...
  public CalculationResultMap evaluate(
    ...
    Date previousPeriod = (Date) context.getFromCache(PREVIOUS_PERIOD);
    ...
    boolean lessThan365Days = (Boolean) parameterValues.get(LESS_THAN_365_DAYS);
    ...
    for (Integer pId : cohort) {
      ...
      if (a != null && b != null) {
        int days = Days.daysIn(new Interval(b.getTime(), a.getTime())).getDays();
        if (lessThan365Days) {
          if (days < YEAR_DAYS) {
            calculationResultMap.put(pId, new BooleanResult(true, this));
          }
        } else {
          if (days >= YEAR_DAYS) {
            calculationResultMap.put(pId, new BooleanResult(true, this));
  ...
  private CalculationResultMap getMostRecentDateFromPatientWhoExperincedITT(
    ...
    sqlPatientDataDefinition.addParameter(
        new Parameter(PREVIOUS_PERIOD, PREVIOUS_PERIOD, Date.class));
    ...
    String query1 =
        " SELECT most_recent.patient_id, Date_add(Max(most_recent.value_datetime), interval ${numDays} day) final_encounter_date "
            ...
            + "                                AND enc.encounter_datetime <= :previousPeriod "
            ...
            + "                            e.voided = 0 and "
            + "                            e.encounter_datetime <= :previousPeriod "
            + "					INNER JOIN obs o on "
