  ...
  public static String getAllPatientsWhoMissedPreviousAppointmentBy28Days(
    ...
    String query =
        "SELECT  "
            ...
            + "        e.encounter_id, "
            + "            p.patient_id, "
            + "            (SELECT MAX(oo.value_datetime) "
            + "             FROM encounter ee  "
            + "             JOIN obs oo ON ee.encounter_id = oo.encounter_id "
            + "             WHERE ee.encounter_datetime = (SELECT  "
            + "                    ee.encounter_datetime "
            + "                FROM "
            + "                    encounter ee "
            + "                INNER JOIN obs oo ON ee.encounter_id = oo.encounter_id "
            + "                WHERE "
            + "                    IF(e.encounter_type = ${masterCardEncounter}, o.value_datetime, e.encounter_datetime) > ee.encounter_datetime "
            + "                        AND ee.voided = 0 "
            + "                        AND oo.voided = 0 "
            ...
            + "                LIMIT 1) AND ee.patient_id = e.patient_id AND ee.encounter_type IN (${adultoSegEncounter} , ${pediatriaSegEncounter}) AND  oo.value_datetime IS NOT NULL) AS prev_consultation_scheduled_date, "
            + "            (SELECT MAX(oo.value_datetime) "
            + "             FROM encounter ee  "
            + "             JOIN obs oo ON ee.encounter_id = oo.encounter_id "
            + "             WHERE ee.encounter_datetime = (SELECT  "
            ...
            + "                        AND ee.encounter_datetime <= :endDate "
            + "                ORDER BY ee.encounter_datetime DESC "
            + "                LIMIT 1) AND ee.patient_id = e.patient_id AND ee.encounter_type = ${pharmacyEncounter} AND  oo.value_datetime IS NOT NULL) AS prev_drug_pickup_scheduled_date_fila, "
            + "            (SELECT  "
            ...
            + "                INNER JOIN obs oo ON ee.encounter_id = oo.encounter_id "
            + "                WHERE "
            + "                    IF(e.encounter_type =  ${masterCardEncounter}, o.value_datetime, e.encounter_datetime) > oo.value_datetime "
            + "                        AND ee.voided = 0 "
            + "                        AND oo.voided = 0 "
            ...
            + "            AND e.encounter_type IN (${adultoSegEncounter} , ${pediatriaSegEncounter}, ${pharmacyEncounter},  ${masterCardEncounter}) "
