  ...
  public CohortDefinition getPatientsRegisteredInFichaAPSSPP() {
    ...
    String query =
        " SELECT p.patient_id "
            ...
            + "        ON e3.patient_id = p.patient_id "
            + "     INNER JOIN encounter e4 "
            + "        ON e4.patient_id = p.patient_id "
            + "     INNER JOIN encounter e5 "
            + "        ON e5.patient_id = p.patient_id "
            + "     INNER JOIN encounter e6 "
            + "        ON e6.patient_id = p.patient_id "
            + "     INNER JOIN encounter e7 "
            + "        ON e7.patient_id = p.patient_id "
            + "     INNER JOIN obs pp1 "
            + "        ON pp1.encounter_id=e1.encounter_id "
            + "     INNER JOIN obs pp2 "
            + "        ON pp2.encounter_id=e2.encounter_id "
            + "     INNER JOIN obs pp3 "
            + "        ON pp3.encounter_id=e3.encounter_id "
            + "     INNER JOIN obs pp4 "
            + "        ON pp4.encounter_id=e4.encounter_id "
            + "     INNER JOIN obs pp5 "
            + "        ON pp5.encounter_id=e5.encounter_id "
            + "     INNER JOIN obs pp6 "
            + "        ON pp6.encounter_id=e6.encounter_id "
            + "     INNER JOIN obs pp7 "
            + "        ON pp7.encounter_id=e7.encounter_id "
            + " WHERE p.voided = 0 "
            + "    AND e1.voided = 0 "
            + "    AND e2.voided = 0 "
            + "    AND e3.voided = 0 "
            ...
            + "    AND pp6.voided = 0 "
            + "    AND pp6.voided = 0 "
            + "    AND e1.encounter_type = ${prevencaoPositivaSeguimentoEncounterType} "
            + "    AND e2.encounter_type = ${prevencaoPositivaSeguimentoEncounterType} "
            + "    AND e3.encounter_type = ${prevencaoPositivaSeguimentoEncounterType} "
            + "    AND e4.encounter_type = ${prevencaoPositivaSeguimentoEncounterType} "
            + "    AND e5.encounter_type = ${prevencaoPositivaSeguimentoEncounterType} "
            + "    AND e6.encounter_type = ${prevencaoPositivaSeguimentoEncounterType} "
            + "    AND e7.encounter_type = ${prevencaoPositivaSeguimentoEncounterType} "
            + "    AND pp1.concept_id = ${pp1Concept} AND pp1.value_coded = ${patientFoundYesConcept} "
            + "    AND pp2.concept_id = ${pp2Concept} AND pp2.value_coded = ${patientFoundYesConcept} "
            + "    AND pp3.concept_id = ${pp3Concept} AND pp3.value_coded = ${patientFoundYesConcept} "
            + "    AND pp4.concept_id = ${pp4Concept} AND pp4.value_coded = ${patientFoundYesConcept} "
            ...
            + "        BETWEEN :startDate AND :endDate "
            + "    AND e4.encounter_datetime "
            + "        BETWEEN :startDate AND :endDate "
            + "    AND e5.encounter_datetime "
            + "        BETWEEN :startDate AND :endDate "
            + "    AND e6.encounter_datetime "
            + "        BETWEEN :startDate AND :endDate "
            + "    AND e7.encounter_datetime "
            + "        BETWEEN :startDate AND :endDate "
            + "    AND e1.location_id = :location "
            + "    AND e2.location_id = :location "
            + "    AND e3.location_id = :location "
            + "    AND e4.location_id = :location "
            + "    AND e5.location_id = :location "
            + "    AND e6.location_id = :location "
            + "    AND e7.location_id = :location ";
