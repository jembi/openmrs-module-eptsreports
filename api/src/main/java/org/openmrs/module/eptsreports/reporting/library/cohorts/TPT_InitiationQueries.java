...
public class TPT_InitiationQueries {
  ...
  public static String getTPTInitiationPatients(
    ...
    String query =
        "  SELECT p.person_id, nid.nid AS NID , p.gender, last_person_name.nome AS name, "
            ...
            + "             AND e.encounter_datetime < pickup.first_pickup_date)  ) TPT ON TPT.patient_id = p.person_id "
            + "  LEFT JOIN (SELECT   p.patient_id,( CASE"
            + "           WHEN p.date_changed IS NOT NULL THEN (SELECT pi.identifier "
            + "                                                 FROM   patient_identifier pi "
            + "                                                 WHERE  pi.voided = 0 AND pi.patient_id = p.patient_id ORDER BY  pi.date_changed   DESC  LIMIT 1) "
            + "           ELSE (SELECT pi.identifier "
            + "                 FROM   patient_identifier pi "
            ...
            + "       ( CASE"
            + "           WHEN p.date_changed IS NOT NULL THEN (SELECT Concat(pn.given_name, ' ', pn.family_name) AS person_name "
            + "                                                 FROM   person_name pn"
            ...
            + "                 FROM   person_name pn"
            + "                 WHERE  pn.voided = 0 AND pn.person_id = p.person_id LIMIT 1) END ) AS nome "
            + " FROM   person_name p "
            ...
            + "   AND hist.value_datetime BETWEEN :startDate AND curdate()    "
            + "           GROUP BY p.patient_id   "
            + "  ) AS breastfeeding   "
  ...
  public static String getTPTInitiationTOTALS(
    ...
    String query =
        "  SELECT (SELECT name FROM location WHERE location_id = :location) AS location, COUNT(TPT.person_id) AS total, CONCAT(DATE_FORMAT(:startDate, '%Y %M %d'),' Até ',DATE_FORMAT(:endDate, '%Y %M %d')) AS period "
            + "  FROM ("
            + "  SELECT p.person_id"
            + "           FROM person p "
            + "           JOIN person_name pn ON p.person_id = pn.person_id "
