...
public class ViralLoadQueries {
  ...
  public static String getPatientsHavingTypeOfDispensationBasedOnTheirLastVlResults(
    ...
    String query =
        " SELECT out_p.patient_id FROM patient pp INNER JOIN encounter ep ON pp.patient_id=ep.patient_id INNER JOIN obs op ON ep.encounter_id=op.encounter_id INNER JOIN ( "
            + " SELECT patient_id, MAX(encounter_datetime) AS max_vl_date_and_max_ficha FROM ( "
            + " SELECT pp.patient_id,ee.encounter_datetime FROM patient pp INNER JOIN encounter ee ON pp.patient_id=ee.patient_id INNER JOIN ( "
