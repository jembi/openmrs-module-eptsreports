...
public class TPTInitiationDataDefinitionQueries {
  ...
  public DataDefinition getPatientsReceivedTPTInTheLastFollowUpConsultation() {
    ...
    String query =
        "SELECT p.patient_id, "
            ...
            + "               AND o2.value_coded IN ( ${656}, ${23954} ) ) "
            + "                 AND ( o3.concept_id = ${165308} "
            + "                   AND o3.value_coded IN ( ${1256}, ${1257} ) ) ) "
            + "UNION "
            + "SELECT p.patient_id, "
            + "       last_fu_consultation.followup_date AS followup_date "
            + "FROM   patient p "
  ...
  public DataDefinition get3HPCompletionDateFichaClínica() {
    ...
    String query =
        "SELECT p.patient_id, "
            ...
            + "                         AND e.location_id = :location "
            + "                         AND e.encounter_type = ${6} "
            + "                         AND o.concept_id = ${1719} AND o.value_coded IN ( ${23954} ) "
            ...
            + "       AND e.encounter_datetime BETWEEN 3hp.first_pickup_date AND Current_date() "
            + "GROUP  BY p.patient_id ";
  ...
  public DataDefinition get3HPCompletionDateonFichaResumo() {
    ...
    String query =
        "SELECT p.patient_id, "
            ...
            + "       AND o2.value_datetime BETWEEN 3hp.first_pickup_date AND Current_date() "
            + "GROUP  BY p.patient_id ";
  ...
  public DataDefinition getExpected3HPCompletionDate() {
    ...
    String query =
        "SELECT last.patient_id,"
            + " DATE_ADD(min(last.first_pickup_date), INTERVAL 86 DAY) as expected_date FROM ( "
            + "   SELECT p.patient_id, first_pickup_date "
            + "   FROM   patient p "
            + "              inner join encounter e ON e.patient_id = p.patient_id "
            + "              inner join obs o ON o.encounter_id = e.encounter_id "
            + "              inner join (SELECT p.patient_id, "
            ...
            + "                            AND e.encounter_datetime >= :startDate "
            + "                            AND e.encounter_datetime <= :endDate "
            + "                          GROUP  BY p.patient_id) AS pickup ON pickup.patient_id = p.patient_id "
            ...
            + "   UNION "
            + "        SELECT p.patient_id, "
            + "               min(e.encounter_datetime) as first_pickup_date "
            + "        FROM   patient p "
            + "            INNER JOIN encounter e ON p.patient_id = e.patient_id "
            + "            INNER JOIN obs o ON e.encounter_id = o.encounter_id "
            + "            INNER JOIN obs o2 ON e.encounter_id = o2.encounter_id "
            ...
            + "        GROUP  BY p.patient_id "
            + "            UNION "
            + "            SELECT p.patient_id, MIN(o2.value_datetime) AS first_pickup_date "
            + "        FROM   patient p "
            + "            INNER JOIN encounter e ON p.patient_id = e.patient_id "
            + "            INNER JOIN obs o ON e.encounter_id = o.encounter_id "
            + "            INNER JOIN obs o2 ON e.encounter_id = o2.encounter_id "
            ...
            + "        UNION "
            + "        SELECT p.patient_id, min(e.encounter_datetime) as first_pickup_date "
            + "        FROM   patient p "
            + "            INNER JOIN encounter e ON p.patient_id = e.patient_id "
            + "            INNER JOIN obs o ON e.encounter_id = o.encounter_id "
            + "            INNER JOIN obs o2 ON e.encounter_id = o2.encounter_id "
            ...
            + "            INNER JOIN encounter e ON e.patient_id = p.patient_id "
            + "            INNER JOIN obs o ON e.encounter_id = o.encounter_id "
            + "            INNER JOIN (  SELECT p.patient_id, Min(e.encounter_datetime) AS start_date "
            + "            FROM patient p "
            + "            INNER JOIN encounter e ON p.patient_id = e.patient_id "
            + "            INNER JOIN obs o ON e.encounter_id = o.encounter_id "
            + "            INNER JOIN obs o2 ON e.encounter_id = o2.encounter_id "
            ...
            + "            ) filt on filt.patient_id = p.patient_id "
            + "            WHERE "
            + "            p.voided = 0 AND e.voided = 0 AND o3.voided = 0 AND o4.voided = 0 AND o5.voided = 0 "
            + "          AND e.location_id = :location "
            + "          AND e.encounter_type = ${6} "
            ...
            + "            ) filt on filt.patient_id = p.patient_id "
            + "            WHERE "
            + "            p.voided = 0 AND e.voided = 0 AND o6.voided = 0 AND o7.voided = 0 "
            + "          AND e.location_id = :location "
            + "          AND e.encounter_type = ${53} "
            ...
            + "          GROUP BY p.patient_id ) "
            + "   GROUP BY p.patient_id) last group by patient_id ";
  ...
  public DataDefinition getDifferencebetweenRegisteredvsExpected3HPCompletionDate() {
    ...
    String query =
        "SELECT    p.patient_id, "
            + "          DATEDIFF(union_tbl.most_recent , threehp_completion.expected_date) AS result "
            + "FROM      patient p "
            + "JOIN ( "
            + "    SELECT 3hp.patient_id, Max(3hp.recent_date) AS most_recent FROM ( "
            + "        SELECT p.patient_id, "
            + "        Max(e.encounter_datetime) AS recent_date "
            + "        FROM   patient p "
            + "        INNER JOIN encounter e ON p.patient_id = e.patient_id "
            ...
            + "                       AND o2.value_datetime BETWEEN 3hp.first_pickup_date AND Current_date() "
            + "                GROUP  BY p.patient_id) 3hp "
            + "    GROUP BY 3hp.patient_id) union_tbl "
            + "ON        union_tbl.patient_id = p.patient_id "
            + "JOIN( SELECT last.patient_id, DATE_ADD(min(last.first_pickup_date), INTERVAL 86 DAY) as expected_date "
            + "      FROM ( SELECT p.patient_id, first_pickup_date "
            + "             FROM   patient p "
            + "                 inner join encounter e ON e.patient_id = p.patient_id "
            + "                 inner join obs o ON o.encounter_id = e.encounter_id "
            ...
            + "                       WHERE  p.patient_id NOT IN (SELECT patient_id "
            + "                                                   FROM   patient p WHERE  p.voided = 0 AND e.voided = 0 AND o.voided = 0 "
            + "                                                     AND e.location_id = :location "
            + "                                                     AND e.encounter_type = ${6} "
            + "                                                     AND o.concept_id = ${1719} "
            + "                                                     AND o.value_coded IN ( ${23954} ) "
            ...
            + "                  Union "
            + "                  SELECT "
            + "                  p.patient_id, min(e.encounter_datetime) as first_pickup_date "
            + "              FROM "
            + "                  patient p "
            + "                  INNER JOIN encounter e ON p.patient_id = e.patient_id "
            + "                  INNER JOIN obs o ON e.encounter_id = o.encounter_id "
            ...
            + "                AND e.encounter_type = ${53} "
            + "                AND o6.concept_id = ${23985} AND o6.value_coded = ${23954} "
            + "                AND o7.concept_id = ${6128} AND o7.value_datetime <= date_sub(filt.start_date, interval 4 month) GROUP BY p.patient_id "
            + "                  ) GROUP BY p.patient_id "
            + "          ) last group by patient_id) threehp_completion "
            + "ON        threehp_completion.patient_id = p. patient_id ";
  ...
  public DataDefinition getPatientsAndIPTInitiationDateOnFichaResumo() {
    ...
    String query =
        "SELECT p.patient_id, "
            ...
            + "        AND o3.value_datetime <= CURRENT_DATE() "
            + "        AND o.value_datetime BETWEEN :startDate AND :endDate "
  ...
  public DataDefinition getPatientsAndIPTCompletioDateOnFichaClinicaOrFichaSeguimento() {
    ...
    String query =
        "SELECT fila.patient_id, "
            + "       Max(fila.encounter_datetime) AS encounter_datetime "
  ...
  public DataDefinition getPatientsAndIPTCompetionDateOnFichaResumo() {
    ...
    String query =
        "SELECT p.patient_id, MAX(o.value_datetime) AS most_recent_date FROM patient p "
            + "	INNER JOIN encounter e ON p.patient_id = e.patient_id "
            + "		INNER JOIN obs o ON e.encounter_id = o.encounter_id "
            + "            INNER JOIN obs o2 ON e.encounter_id = o2.encounter_id "
            + "WHERE p.voided = 0 "
            ...
            + "AND e.location_id = :location "
            + "AND e.encounter_type = ${53} "
            + "AND ((o.concept_id = ${6129} OR o2.concept_id = ${23985} AND o2.value_coded = ${23954})) "
