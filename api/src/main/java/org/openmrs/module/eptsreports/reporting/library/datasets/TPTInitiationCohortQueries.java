...
public class TPTInitiationCohortQueries {
    ...
    public CohortDefinition getBaseCohort() {
        ...
        cd.addSearch(
                "A",
                EptsReportUtils.map(
                        A, "startDate=${startDate},endDate=${endDate},location=${location}"));
        cd.addSearch(
                "B",
                EptsReportUtils.map(
                        B, "startDate=${startDate},endDate=${endDate},location=${location}"));
    ...
    public CohortDefinition getPatientsIPTStart1WithUltimaProxilaxiaIsonazida() {
        ...
        SqlCohortDefinition sqlCohortDefinition = new SqlCohortDefinition();
        sqlCohortDefinition.setName("IPT start 1 - Patients with ficha resumo and ultima profilaxia Isoniazida");
        sqlCohortDefinition.addParameter(new Parameter("startDate", "startDate", Date.class));
        sqlCohortDefinition.addParameter(new Parameter("endDate", "endDate", Date.class));
        sqlCohortDefinition.addParameter(new Parameter("location", "location", Location.class));
     ...
     * <blockquote>
     *
     * <p> Select all patients with Ficha clinica (encounter type 6) with “Profilaxia INH” (concept id 6122) with value code “Inicio”
     * (concept id 1256) and encounter datetime between start date and end date</p>
     *
     * </blockquote>
     *
     * @return {@link CohortDefinition}
     ...
     * <blockquote>
     *
     * <p> Select all patients with Ficha clinica or Ficha Pediatrica (encounter type 6,9) with “Profilaxia com
     * INH” (concept id 6128) and value datetime is not null and between start date and end date</p>
     *
     * </blockquote>
     *
     * @return {@link CohortDefinition}
