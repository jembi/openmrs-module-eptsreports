...
public class TxPvlsBySourceClinicalOrFichaResumoCohortQueries {
  ...
  public CohortDefinition getBreastfeedingWomenWithViralSuppressionForFichaMestreNumerator() {
    ...
    cd.addSearch(
        "breastfeeding",
        EptsReportUtils.map(
            getBreastfeedingPatientsWithClinicalViralLoadResults(),
            "endDate=${endDate},location=${location}"));
  ...
  public CohortDefinition getPregnantWomenWithViralLoadSuppressionForLabAndFsrNumerator() {
    ...
    cd.addSearch(
        "pregnant",
        EptsReportUtils.map(
            getPregnantWomanWithClinicalVLResult(), "endDate=${endDate},location=${location}"));
   ...
   * @return {@link CohortDefinition}
   */
  public CohortDefinition getPregnantWomanWithClinicalVLResult() {
    ...
    String query =
        "SELECT pregnant.patient_id "
            ...
            + "                                                                     OR         ( "
            + "                                                                                           e.encounter_type =${53} "
            + "                                                                                AND        o.obs_datetime BETWEEN timestampadd(month, -12, :endDate) AND        :endDate ) ) "
            + "                                                          AND        ( ( "
            + "                                                                                           o.concept_id = ${856} "
            ...
            + "                  WHERE      pg.pregnancy_date BETWEEN timestampadd(month, -9, vl.last_date) AND        vl.last_date "
            ...
            + "                  AND        ( "
            + "                                        o.concept_id = ${5599} "
            + "                             AND        o.value_datetime BETWEEN timestampadd(month, -18, pregnant.last_vl) AND        pregnant.last_vl ) "
            + "                  AND        e.location_id = :location "
            ...
            + "                  AND        e.encounter_datetime BETWEEN timestampadd(month, -18, pregnant.last_vl) AND        pregnant.last_vl "
            + "                  AND        o.voided = 0 "
            + "                  AND        e.voided = 0 "
            + "                  UNION "
            + "                  SELECT     e.patient_id "
            ...
            + "                  AND        ps.start_date BETWEEN timestampadd(month, -18 , pregnant.last_vl) AND        pregnant.last_vl "
            + "                  UNION "
            + "                  SELECT     e.patient_id "
            + "                  FROM       encounter e "
            + "                  INNER JOIN obs o "
            ...
            + "                  AND        o.obs_datetime BETWEEN timestampadd(month, -18 , pregnant.last_vl) AND        pregnant.last_vl "
            + "                  AND        e.voided = 0 "
            + "                  AND        o.voided = 0 "
            + "                  UNION "
            + "                  SELECT     e.patient_id "
            ...
            + "                  AND        cast(o2.value_datetime AS date) BETWEEN timestampadd(month, -18, pregnant.last_vl) AND        pregnant.last_vl "
            + "                  AND        pregnant.patient_id = e.patient_id "
   ...
   * </ul>
   */
  public CohortDefinition getBreastfeedingPatientsWithClinicalViralLoadResults() {
    SqlCohortDefinition sqlCohortDefinition = new SqlCohortDefinition();
    sqlCohortDefinition.setName(" Patients disaggregation - breastfeeding for Clinical VL");
    sqlCohortDefinition.addParameter(new Parameter("endDate", "End Date", Date.class));
    sqlCohortDefinition.addParameter(new Parameter("location", "Location", Location.class));
    ...
    String query =
        "SELECT breastfeeding.patient_id "
            ...
            + "                              AND o.voided = 0) vl "
            + "               GROUP  BY vl.patient_id) vl "
            + "              INNER JOIN(SELECT lactantes.patient_id, "
            ...
            + "                                        INNER JOIN obs hist "
            + "                                                ON "
            + "                                        e.encounter_id = hist.encounter_id "
            + "                                 WHERE  p.voided = 0 "
            + "                                        AND e.voided = 0 "
            + "                                        AND o.voided = 0 "
            + "                                        AND pe.voided = 0 "
            ...
            + "                                 UNION "
            + "                                 SELECT p.patient_id, "
            + "                                        Cast(o2.value_datetime AS date) AS "
            + "                                        last_date "
            + "                                 FROM   patient p "
            + "                                        INNER JOIN encounter e "
            + "                                                ON e.patient_id = p.patient_id "
            ...
            + "                                        AND o.value_coded = ${1065} "
            + "                                        AND o2.concept_id = ${23821} "
            + "                                        AND Cast(o2.value_datetime AS date) <= "
            + "                                            :endDate "
            + "                                        AND p2.gender = 'F' "
            + "                                        AND p.voided = 0 "
            + "                                        AND e.voided = 0 "
            + "                                        AND o.voided = 0 "
            + "                                        AND o2.voided = 0 "
            + "                                        AND e.location_id = :location)lactantes) "
            + "                        bf "
            + "                      ON vl.patient_id = bf.patient_id "
            + "       WHERE  bf.breastfeeding_date BETWEEN "
            + "              Timestampadd(month, -18, vl.last_date) AND "
            + "              vl.last_date "
            ...
            + "                          AND e.encounter_datetime BETWEEN "
            + "                              Timestampadd(month, -9, "
            + "                              breastfeeding.last_vl) AND "
            + "                              breastfeeding.last_vl "
            + "                   UNION "
            + "                   SELECT e.patient_id "
            + "                   FROM   encounter e "
            + "                          INNER JOIN obs o "
