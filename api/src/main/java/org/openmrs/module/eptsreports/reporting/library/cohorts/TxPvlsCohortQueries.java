...
public class TxPvlsCohortQueries {
  ...
  public CohortDefinition getBreastfeedingWomenWhoHaveViralSuppression() {
    ...
    cd.addSearch(
        "breastfeeding",
        EptsReportUtils.map(getBreastfeedingPatients(),
  ...
  public CohortDefinition getBreastfeedingPatients() {
    ...
    String query =
            "SELECT p.patient_id "
                    ...
                    + "       inner join(SELECT ultima.patient_id, "
                    + "                         Max(ultima.last_date) AS ultima "
                    + "                  FROM   (SELECT p.patient_id, "
                    ...
                    + "                                     AND "
                    + "                                                            :endDate "
                    + "                          GROUP  BY p.patient_id) ultima "
                    + "                  GROUP  BY ultima.patient_id)recente "
                    + "               ON recente.patient_id = p.patient_id "
                    + "WHERE  p2.gender = 'F' "
                    + "       AND e.encounter_type IN ( ${5}, ${6} ) "
                    + "       AND e.location_id = :location "
                    + "       AND o.concept_id = ${5599} "
                    + "       AND o.value_datetime BETWEEN Date_sub(recente.ultima, interval 18 month) "
                    + "                                    AND "
                    + "                                    recente.ultima "
                    + "       AND o.voided = 0 "
                    + "       AND p.voided = 0 "
                    ...
                    + "       AND e.encounter_datetime BETWEEN Date_sub(recente.ultima, "
                    ...
                    + "                   GROUP  BY ultima.patient_id)recente "
                    + "               ON recente.patient_id = p.patient_id "
                    + "WHERE  ps.patient_state_id = ${27} "
                    + "       AND pp.program_id = ${8} "
                    + "       AND ps.start_date BETWEEN Date_sub(recente.ultima, interval 18 month) AND "
                    + "                                 recente.ultima "
                    + "UNION "
                    + "SELECT p.patient_id "
                    + "FROM   patient p "
                    ...
                    + "       AND p.voided = 0 "
                    + "       AND e.voided = 0 "
                    + "       AND e.encounter_datetime <= recente.ultima "
                    + "GROUP  BY p.patient_id "
                    + "UNION "
                    + "SELECT p.patient_id "
                    + "FROM   patient p "
                    ...
                    + "       AND o2.obs_datetime BETWEEN Date_sub(recente.ultima, interval 18 month) AND "
                    + "                                   recente.ultima "
                    + "GROUP  BY p.patient_id";
