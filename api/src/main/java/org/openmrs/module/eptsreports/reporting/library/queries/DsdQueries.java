  ...
  public static SqlCohortDefinition
    ...
    String query =
        "SELECT "
            ...
            + "        FROM "
            + "          encounter ee "
            + "          INNER JOIN obs oo1 ON ee.encounter_id = oo1.encounter_id "
            + "          INNER JOIN ( "
            + "            SELECT "
            + "              pp1.patient_id, "
            + "              MAX(ee1.encounter_datetime) AS last_encounter "
            + "            FROM "
            + "              patient pp1 "
            + "              INNER JOIN encounter ee1 ON ee1.patient_id = pp1.patient_id "
            + "            WHERE "
            + "              pp1.voided = 0 "
            + "              AND ee1.voided = 0 "
            + "              AND ee1.location_id = :location "
            + "              AND ee1.encounter_type = 18 "
            + "              AND ee1.encounter_datetime <= :endDate "
            + "            GROUP BY "
            + "              pp1.patient_id "
            + "          ) last_fila ON ee.patient_id = last_fila.patient_id "
            + "        WHERE "
  ...
  public static SqlCohortDefinition
    ...
    String query =
        ""
            ...
            + "         INNER JOIN obs otype ON otype.encounter_id = e.encounter_id  "
            + "         INNER JOIN obs ostate ON ostate.encounter_id = e.encounter_id  "
            + "         INNER JOIN (SELECT last_encounter.patient_id, Max(last_encounter.last_encounter_datetime) AS last_encounter "
            + "                     FROM (SELECT p.patient_id, Max(e.encounter_datetime) AS last_encounter_datetime "
            + "                           FROM patient p "
            + "                                    INNER JOIN encounter e ON e.patient_id = p.patient_id "
            + "                           WHERE p.voided = 0 "
            + "                             AND e.voided = 0 "
            ...
            + "                           UNION "
            + " "
            + "                           SELECT p.patient_id, MAX(o.value_datetime) AS last_encounter_datetime "
            + "                           FROM patient p "
            + "                                    INNER JOIN encounter e ON e.patient_id = p.patient_id "
            ...
            + "                             AND e.voided = 0 "
            + "                             AND o.voided = 0 "
            + "                           GROUP BY p.patient_id) last_encounter "
