...
public class PregnantQueries {
  ...
  public static String getBreastfeedingPregnantWhileOnArt(
      int breastFeedingConcept,
      int yesConcept,
      int eddConcept,
      int adultInitailEncounter,
    ...
    return "  SELECT patient_id FROM (  "
        ...
        + " ) AS breastfeeding "
        + " INNER JOIN "
        + " (SELECT patient_id, MAX(pregnancy_date) AS date FROM "
        + " (SELECT p.patient_id , MAX(e.encounter_datetime) AS pregnancy_date "
        + " FROM patient p "
        + " INNER JOIN person pe ON p.patient_id=pe.person_id "
        + " INNER JOIN encounter e ON p.patient_id=e.patient_id "
        ...
        + " WHERE pregnant.date = breastfeeding.last_date"
        + " GROUP BY patient_id) AS LIST ";
  }
}
