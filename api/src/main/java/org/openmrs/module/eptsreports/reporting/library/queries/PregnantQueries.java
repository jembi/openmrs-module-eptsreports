  ...
  public static String getBreastfeedingWhileOnArt(
    ...
    String query =
        "  SELECT list.patient_id FROM (  "
            ...
            + " AND e.encounter_type = "
            + fichaResumo
            + " AND historical_date.value_datetime between :onOrAfter AND :onOrBefore AND e.location_id= :location AND pe.gender='F' GROUP BY p.patient_id "
            + " UNION "
            + " Select p.patient_id,  MAX(e.encounter_datetime) as pregnancy_date "
            + " FROM patient p "
            + " INNER JOIN person pe ON p.patient_id=pe.person_id "
            ...
            + " WHERE pp.program_id=  "
            + etvProgram
            + " AND pp.voided=0 AND pp.date_enrolled between  :onOrAfter AND :onOrBefore AND pp.location_id= :location AND pe.gender='F' GROUP BY pp.patient_id "
            + " UNION "
            + " SELECT p.patient_id,  MAX(o.value_datetime) as pregnancy_date  FROM patient p "
            + " INNER JOIN person pe ON p.patient_id=pe.person_id "
            + " INNER JOIN encounter e ON p.patient_id=e.patient_id "
  ...
  public static String getPregnantWhileOnArtDSD(
    ...
    String query =
        "Select max_pregnant.patient_id from "
            ...
            + adultSegEncounter
            + ") "
            + "     AND o.value_datetime BETWEEN DATE_SUB(:endDate, INTERVAL 18 MONTH) AND :endDate AND e.location_id=:location AND pe.gender='F' "
            + "      GROUP BY p.patient_id "
            + "      UNION "
            + "     SELECT     p.patient_id, MAX(e.encounter_datetime) AS last_date "
            + "      FROM patient p "
            ...
            + adultSegEncounter
            + ") "
            + "      AND e.encounter_datetime BETWEEN DATE_SUB(:endDate, INTERVAL 18 MONTH) AND :endDate AND e.location_id=:location AND pe.gender='F' "
            + "      GROUP BY p.patient_id "
            + "      UNION "
            + "      SELECT     p.patient_id, MAX(e.encounter_datetime) AS last_date "
            + "      FROM patient p "
            ...
            + "      AND ps.state= "
            + gaveBirthPatientState
            + "      AND pp.voided=0 AND ps.start_date BETWEEN DATE_SUB(:endDate, INTERVAL 18 MONTH) AND :endDate AND pp.location_id=:location AND pe.gender='F' "
            + "      GROUP BY pp.patient_id "
            + "     UNION "
            + "      SELECT p.patient_id, MAX(hist.value_datetime) AS last_date "
            + "      FROM patient p "
  ...
  public static String getBreastfeedingWhileOnArtDSD(
    ...
    String query =
        "  SELECT list.patient_id FROM (  "
            ...
            + ","
            + adultSegEncounter
            + " ) AND e.encounter_datetime between DATE_SUB(:onOrBefore, INTERVAL 9 MONTH) AND :onOrBefore AND e.location_id= :location AND pe.gender='F' GROUP BY p.patient_id "
            + " UNION "
            + " select p.patient_id, MAX(historical_date.value_datetime) as pregnancy_date FROM patient p "
            + " INNER JOIN person pe ON p.patient_id=pe.person_id "
            + " INNER JOIN encounter e ON p.patient_id=e.patient_id "
            ...
            + " AND e.encounter_type = "
            + fichaResumo
            + " AND historical_date.value_datetime between DATE_SUB(:onOrBefore, INTERVAL 9 MONTH) AND :onOrBefore AND e.location_id= :location AND pe.gender='F' GROUP BY p.patient_id "
            + " UNION "
            + " Select p.patient_id,  MAX(e.encounter_datetime) as pregnancy_date "
            + " FROM patient p "
            + " INNER JOIN person pe ON p.patient_id=pe.person_id "
            ...
            + " WHERE pp.program_id=  "
            + etvProgram
            + " AND pp.voided=0 AND pp.date_enrolled between  DATE_SUB(:onOrBefore, INTERVAL 9 MONTH) AND :onOrBefore AND pp.location_id= :location AND pe.gender='F' GROUP BY pp.patient_id "
            + " UNION "
            + " SELECT p.patient_id,  MAX(o.value_datetime) as pregnancy_date  FROM patient p "
            + " INNER JOIN person pe ON p.patient_id=pe.person_id "
            + " INNER JOIN encounter e ON p.patient_id=e.patient_id "
            ...
            + " AND e.encounter_type =  "
            + adultSegEncounter
            + " AND pe.gender='F' AND o.value_datetime BETWEEN DATE_SUB(:onOrBefore, INTERVAL 9 MONTH) AND :onOrBefore GROUP BY p.patient_id) as pregnancy  "
            + " GROUP BY pregnancy.patient_id) AS pregnant_table "
            + " ON pregnant_table.patient_id = breastfeeding.patient_id "
            + " WHERE (breastfeeding.last_date > pregnant_table.pregnancy_date "
            + " OR pregnant_table.pregnancy_date IS NULL)"
