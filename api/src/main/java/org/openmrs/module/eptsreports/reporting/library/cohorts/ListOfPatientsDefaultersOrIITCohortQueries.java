  ...
  public DataDefinition getNumberOfDaysOfDelay() {
    ...
    String query =
        "SELECT  DISTINCT p.patient_id,"
            ...
            + "    INNER JOIN"
            + "      (SELECT p.patient_id, DATE_ADD(o.value_datetime, interval 30 DAY ) AS last_scheduled"
            + "       FROM   patient p"
            + "          INNER JOIN encounter e ON p.patient_id = e.patient_id"
            + "          INNER JOIN obs o ON e.encounter_id = o.encounter_id"
            + "          INNER JOIN (SELECT p.patient_id, Max(o.value_datetime) AS most_valuedatetime"
            + "                      FROM   patient p"
            + "                        INNER JOIN encounter e ON p.patient_id =  e.patient_id"
            + "                        INNER JOIN obs o ON e.encounter_id = o.encounter_id"
            + "                        INNER JOIN obs o2 ON e.encounter_id = o2.encounter_id"
            ...
            + "              AND e.voided = 0"
            + "              AND o.voided = 0"
            + "              AND e.location_id = :location) AS last_mastercard_scheduled ON p.patient_id = last_mastercard_scheduled.patient_id";
