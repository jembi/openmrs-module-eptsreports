  ...
  public DataDefinition getTypeOfDispensation() {
    ...
    String query =
        "  SELECT p.patient_id, CASE                                                                                                                               "
            + "                           WHEN mensal.patient_id IS NOT NULL THEN 'Dispensa Mensal'  "
            + "                           WHEN trimestral.patient_id IS NOT NULL THEN 'Dispensa Trimestral'  "
            + "                           WHEN semestral.patient_id IS NOT NULL THEN 'Dispensa Semestral' ELSE '' END  "
            + "  FROM patient p  "
            + "           LEFT JOIN  "
            ...
            + "                                              AND o.concept_id = ${5096}  "
            + "                                              AND DATE(ex.encounter_datetime) <= :endDate "
            ...
            + "                                         AND DATE(list.encounter_datetime) <= :endDate  "
            + "                                         AND list.voided = 0  "
            ...
            + "                                         AND list.location_id = :location "
            + "                                         AND list.patient_id = en.patient_id) "
            + "         GROUP BY en.patient_id) AS mensal ON mensal.patient_id = p.patient_id  "
            + "           LEFT JOIN  "
            + "       (SELECT    en.patient_id "
            + "        FROM  "
            + "            (SELECT "
            ...
            + "    "
            + "                 WHERE DATE(e.encounter_datetime) <= :endDate "
            + "                   AND e.location_id = :location  "
            + "                   AND e.voided = 0 "
            + "                   AND e.encounter_type = ${18} "
            + "                   AND e.patient_id = en.patient_id "
            ...
            + "                                        AND list.location_id = :location  "
            + "                                        AND list.patient_id = en.patient_id)  "
            + "        GROUP BY en.patient_id) AS trimestral ON trimestral.patient_id = p.patient_id "
            + "           LEFT JOIN  "
            + "       ( SELECT en.patient_id "
            + "         FROM "
            + "             (SELECT  "
            ...
            + "                                         AND list.location_id = :location "
            + "                                         AND list.patient_id = en.patient_id) "
            + "         GROUP BY en.patient_id) AS semestral ON semestral.patient_id = p.patient_id; ";
