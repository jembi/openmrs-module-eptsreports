  ...
  public CohortDefinition getLastARVRegimen() {
    ...
    String sql =
        "   SELECT last_next_scheduled_pick_up.patient_id     "
            ...
            + "                       AND e.encounter_datetime = most_recent.e_datetime "
            + "                       AND TIMESTAMPDIFF(DAY, DATE(o.value_datetime), DATE(:endDate)) >= :minDay     "
            + "                       AND TIMESTAMPDIFF(DAY, DATE (o.value_datetime), DATE(:endDate)) <= :maxDay      "
            ...
            + "                        GROUP  BY p.patient_id) last_arv on p.patient_id = last_arv.patient_id     "
            + "                     WHERE TIMESTAMPDIFF(DAY, last_arv.result_Value, :endDate) >= :minDay      "
            + "                       AND TIMESTAMPDIFF(DAY, last_arv.result_Value, :endDate) <= :maxDay   "
