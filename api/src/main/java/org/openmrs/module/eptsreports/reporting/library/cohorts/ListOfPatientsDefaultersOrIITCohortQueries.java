  ...
  public CohortDefinition getLastARVRegimen() {
    ...
    String sql =
        "  SELECT last_next_scheduled_pick_up.patient_id   "
            ...
            + "           AND e.encounter_datetime = most_recent.e_datetime    "
            + "      ) last_next on p.patient_id = last_next.patient_id    "
            + "       WHERE TIMESTAMPDIFF(DAY,DATE (last_next.result_Value), DATE (:endDate)) >= :minDay   "
            + "           AND TIMESTAMPDIFF(DAY,DATE (last_next.result_Value), DATE (:endDate)) <= :maxDay     "
            + "         UNION      "
            + "         SELECT p.patient_id from patient p INNER JOIN  "
            + "         (  "
            + "             SELECT p.patient_id,(o.value_datetime) AS  result_Value    "
            ...
            + "         ) last_arv on p.patient_id = last_arv.patient_id   "
            + "         WHERE  "
            + "            TIMESTAMPDIFF(DAY,DATE_ADD(last_arv.result_Value, INTERVAL 30 DAY), :endDate) >= :minDay    "
            + "           AND TIMESTAMPDIFF(DAY,DATE_ADD(last_arv.result_Value, INTERVAL 30 DAY), :endDate) <= :maxDay "
  ...
  public DataDefinition getNumberOfDaysOfDelay() {
    ...
    String query =
        "SELECT  DISTINCT p.patient_id, "
            ...
            + "        AND e.encounter_type = ${18} "
            + "        AND e.encounter_datetime <= :endDate "
            + "      GROUP  BY p.patient_id) AS last_famacia ON p.patient_id = last_famacia.patient_id "
            + "         INNER JOIN "
            + "     (SELECT p.patient_id, Max(o1.value_datetime) AS last_pickup "
            + "      FROM   patient p "
            + "                 INNER JOIN encounter e ON p.patient_id =  e.patient_id "
            ...
            + "        AND e.location_id = :location "
            + "        AND e.encounter_type = ${52} "
            + "        AND (o1.concept_id = ${23866} AND o1.value_datetime <= :endDate) "
            + "        AND (o2.concept_id = ${23865} AND o2.value_coded = ${1065}) "
            + "      GROUP  BY p.patient_id) AS last_mastercard_pick ON p.patient_id = last_mastercard_pick.patient_id "
            + "         INNER JOIN "
            + "     (SELECT p.patient_id, o.value_datetime  AS last_scheduled "
            + "      FROM   patient p "
            + "                 INNER JOIN encounter e ON p.patient_id = e.patient_id "
            + "                 INNER JOIN obs o ON e.encounter_id = o.encounter_id "
            + "                 INNER JOIN (SELECT p.patient_id,  Max(e.encounter_datetime) AS last_famacia "
            + "                             FROM   patient p "
            + "                                        INNER JOIN encounter e ON p.patient_id = e.patient_id "
            + "                                        INNER JOIN obs o ON e.encounter_id = o.encounter_id "
            + "                             WHERE  p.voided = 0 "
            ...
            + "        AND o.voided = 0 "
            + "        AND e.encounter_type = ${18} "
            + "        AND e.encounter_datetime =  most_recent.last_famacia "
            + "        AND o.concept_id =  ${5096} "
            + "        AND e.location_id = :location) AS last_farmacia_scheduled ON p.patient_id = last_farmacia_scheduled.patient_id "
            + "         INNER JOIN "
            + "     (SELECT p.patient_id, DATE_ADD(Max(o.value_datetime), interval 30 DAY) AS last_scheduled "
            + "      FROM   patient p "
            + "                 INNER JOIN encounter e ON p.patient_id =  e.patient_id "
            + "                 INNER JOIN obs o ON e.encounter_id = o.encounter_id "
            + "                 INNER JOIN obs o2 ON e.encounter_id = o2.encounter_id "
