  ...
  public static SqlCohortDefinition getPatientsWithPickupOnFilaBasedOnLastVl12Months(
    ...
    String query =
        "SELECT p.patient_id FROM patient p "
            ...
            + " AND e.location_id = :location "
            + ")max_vl GROUP BY patient_id)mds_encounter ON pp.patient_id= mds_encounter.patient_id WHERE ee.encounter_type=${18} "
            + " AND ee.encounter_datetime BETWEEN Date_add(vl_max_date, interval - 12 MONTH) "
            + " AND  Date_add(vl_max_date, interval - 1 DAY) AND pp.voided=0 AND ee.voided=0 "
            + " GROUP BY pp.patient_id)ficha_max_date_tbl ON p.patient_id=ficha_max_date_tbl.patient_id "
