...
public class ListOfPatientsOnAdvancedHivIllnessQueries {
  ...
  public String getVLoadResultAndMostRecent() {
    return "SELECT p.patient_id, "
        ...
        + "                                                                                                                                      limit 1 ), (if(e.encounter_type = 51 and o2.value_coded=1306, 'NIVEL BAIXO DE DETECÇÃO', (IF (o2.value_coded = 23814,'INDETECTÁVEL', (SELECT cn.name FROM concept_name cn where concept_id = o2.value_coded AND locale = 'pt' limit 1) ))     ))) )) AS "
        + "       viral_load "
        + "FROM   patient p "
        + "       INNER JOIN encounter e "
  ...
  public String getMostRecentVLBeforeLastVLDate() {
    return "SELECT p.patient_id, "
        ...
        + "               ON e.encounter_id = o2.encounter_id "
        + "       INNER JOIN ("
        + " SELECT result_date.patient_id, MAX(result_date.most_recent) as last_vl FROM ( "
