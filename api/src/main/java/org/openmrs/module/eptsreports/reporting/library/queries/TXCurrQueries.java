  ...
  public static String getPatientWithoutScheduledDrugPickupDateMasterCardAmdArtPickup(
    ...
    String query =
        "SELECT pat.patient_id FROM patient pat WHERE pat.voided=0 AND pat.patient_id NOT IN( "
            ...
            + " AND pat.voided=0 AND e.voided=0 AND e.location_id=:location AND e.encounter_type IN(${adultoSeguimentoEncounterType}) GROUP BY pat.patient_id)qa "
            ...
            + " AND e1.voided=0 AND e1.encounter_type IN(${adultoSeguimentoEncounterType}) AND e1.location_id=:location AND o1.value_datetime IS NOT NULL "
            + " AND o1.voided=0 AND o1.concept_id IN(${returnVisitDateForArvDrugConcept}, ${returnVisitDateConcept}) AND o1.location_id =:location "
            + " UNION "
            + " SELECT qb.patient_id FROM( "
            + " SELECT pat.patient_id,MAX(e.encounter_datetime) AS encounter_datetime FROM patient pat "
            + " INNER JOIN encounter e ON pat.patient_id=e.patient_id WHERE e.encounter_datetime<=:onOrBefore "
            + " AND pat.voided=0 AND e.voided=0 AND e.location_id=:location AND e.encounter_type IN(${ARVPediatriaSeguimentoEncounterType}) GROUP BY pat.patient_id)qb "
            + " INNER JOIN encounter e1 ON qb.patient_id=e1.patient_id "
            + " INNER JOIN obs o1 ON e1.encounter_id=o1.encounter_id "
            + " WHERE qb.encounter_datetime=e1.encounter_datetime AND e1.encounter_datetime<=:onOrBefore "
            + " AND e1.voided=0 AND e1.encounter_type IN(${ARVPediatriaSeguimentoEncounterType}) AND e1.location_id=:location AND o1.value_datetime IS NOT NULL "
            + " AND o1.voided=0 AND o1.concept_id IN(${returnVisitDateForArvDrugConcept}, ${returnVisitDateConcept}) AND o1.location_id = :location "
            + " UNION "
            + " SELECT qc.patient_id FROM( "
            + " SELECT pat.patient_id,MAX(e.encounter_datetime) AS encounter_datetime FROM patient pat "
            + " INNER JOIN encounter e ON pat.patient_id=e.patient_id WHERE e.encounter_datetime<=:onOrBefore "
            + " AND pat.voided=0 AND e.voided=0 AND e.location_id=:location AND e.encounter_type IN(${aRVPharmaciaEncounterType}) GROUP BY pat.patient_id)qc "
            + " INNER JOIN encounter e1 ON qc.patient_id=e1.patient_id "
            + " INNER JOIN obs o1 ON e1.encounter_id=o1.encounter_id "
            + " WHERE qc.encounter_datetime=e1.encounter_datetime AND e1.encounter_datetime<=:onOrBefore "
            + " AND e1.voided=0 AND e1.encounter_type IN(${aRVPharmaciaEncounterType}) AND e1.location_id=:location AND o1.value_datetime IS NOT NULL "
            + " AND o1.voided=0 AND o1.concept_id IN(${returnVisitDateForArvDrugConcept}, ${returnVisitDateConcept}) AND o1.location_id = :location "
            + " UNION "
  ...
  public static String getPatientWhoAfterMostRecentDateHaveDrusPickupOrConsultationComposition(
    ...
    String query =
        "select most_recent.patient_id from  "
            ...
            + "    GROUP BY  max_date.patient_id"
            + "   UNION  "
            + "SELECT last9.patient_id , max(last9.encounter_datetime) AS lastdate "
            + " FROM ( "
            ...
            + "   UNION  "
            + " SELECT  last10.patient_id, max(last10.encounterdatetime) "
            + " FROM ( "
            ...
            + "   UNION  "
            + " SELECT  last11.patient_id, max(last11.encounterdatetime) "
            + " FROM ( "
            + " 	SELECT  p.patient_id , e.encounter_datetime  AS encounterdatetime "
            + "     FROM patient p  "
            + "          INNER JOIN encounter e  "
