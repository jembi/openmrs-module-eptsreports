  ...
  public static String getPatientWhoAfterMostRecentDateHaveDrusPickupOrConsultationComposition(
    ...
    String query =
        "select most_recent2.patient_id from ( "
            ...
            + "   UNION  "
            + "   SELECT p.person_id patient_id, max(p.death_date) common_date  "
            + "   FROM person p "
            + "    WHERE p.dead=1 "
            + "     AND p.death_date <=  :onOrBefore "
            + "     AND p.voided=0 "
            ...
            + "    GROUP BY  max_date.patient_id"
            + "   UNION  "
            + "SELECT last9.patient_id , max(last9.encounter_datetime) common_date "
            + " FROM ( "
            ...
            + "        WHERE e.voided=0 "
            + "            AND obss.voided=0 "
            + "            AND (e.encounter_type IN (${adultoSeguimento},${aRVPediatriaSeguimento},${aRVPharmacia}) AND  e.encounter_datetime >  most_recent2.common_date  AND e.encounter_datetime <= :onOrBefore ) OR "
