...
public class QualityImprovement2020CohortQueries {
  ...
  public CohortDefinition getDqueryFromCat3() {
    ...
    + "                GROUP BY p_diagnostico.patient_id) AS diag ON diag.patient_id = p.patient_id   "
    + "        WHERE   "
    + "            p.voided = 0 AND e.voided = 0   "
    + "                AND e.location_id = :location   "
    + "                AND e.encounter_type = ${adultoSeguimentoEncounterType}   "
    + "                AND e.encounter_datetime >= diag.data_diagnostico   "
    + "                AND e.encounter_datetime <= DATE_ADD(diag.data_diagnostico, INTERVAL 7 DAY)   "
    + "      GROUP BY p.patient_id) AS final_diag   "
    + "      UNION   "
    + "      SELECT    "
