  ...
  public CohortDefinition getApssBetween20and33DaysAfterArtStart(Integer minDays, Integer maxDays) {
    ...
    String query =
        "SELECT p.patient_id "
            ...
            + "                    ON p.patient_id = e.patient_id "
            + "INNER JOIN ( "
            + "    SELECT p.patient_id, MIN(e.encounter_datetime) as apss_date "
            + "    FROM patient p "
            + "             INNER JOIN encounter e "
            + "                        ON p.patient_id = e.patient_id "
            + "    WHERE  p.voided = 0 "
            + "      AND e.voided = 0 and p.patient_id in (42877,42920) "
            + "      AND e.encounter_type = 35 "
            + "      and e.encounter_datetime <= :endDate "
            + "      AND e.location_id = 398 "
            + "    GROUP  BY p.patient_id "
            + ") first_apss on p.patient_id =  first_apss.patient_id"
            + "         INNER JOIN ( "
            ...
            + "  AND e.encounter_type = ${35} "
            + "  AND e.location_id = :location "
            + "  AND e.encounter_datetime = first_apss.apss_date "
  ...
  public CohortDefinition getApssBetween20and33DaysAfterFirstApss(
    ...
    String query =
        "SELECT p.patient_id "
            ...
            + "                    ON p.patient_id = e.patient_id "
            + "         INNER JOIN ( "
            + "    SELECT p.patient_id, first_apss.apss_date as first_consultation "
            + "     FROM patient p "
            + "         INNER JOIN encounter e "
            + "                    ON p.patient_id = e.patient_id "
            + "INNER JOIN ( "
