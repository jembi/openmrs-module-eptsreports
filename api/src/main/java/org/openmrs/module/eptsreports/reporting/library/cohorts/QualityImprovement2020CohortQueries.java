...
public class QualityImprovement2020CohortQueries {
  ...
  //   *
  //   * <ul>
  //   *   <li>G - Select all patients who have 3 APSS&PP (encounter type 35) consultations in 99
  // days
  //   *       after Viral Load Result (the oldest date from B2) following the conditions:
  //   *   <li>
  //   *       <p>G1 - One Consultation (Encounter_datetime (from encounter type 35)) on the same
  // date
  //   *       when the Viral Load with >=1000 result was recorded (the oldest date from B2) AND
  //   *   <li>
  //   *       <p>G2 -Another consultation (Encounter_datetime (from encounter type 35) > “Viral
  // Load
  ...
  //   *       Date” (date from G2, the oldest from G2)+20days and <=“Second Date” (date from G2,
  // the
  //   *       oldest one)+33days.
  //   * </ul>
  //   *
  ...
  //            + "      WHERE   "
  //            + "          p.voided = 0 AND e.voided = 0   "
  //            + "              AND e.encounter_type = ${35}) visit1 ON visit1.patient_id =
  // p.patient_id   "
  //            + "          INNER JOIN   "
  //            + "      (SELECT    "
  ...
  //            + "      WHERE   "
  //            + "          p.voided = 0 AND e.voided = 0   "
  //            + "              AND e.encounter_type = ${35}) visit2 ON visit2.patient_id =
  // p.patient_id   "
  //            + "          INNER JOIN   "
  //            + "      (SELECT    "
  ...
  //            + "      WHERE   "
  //            + "          p.voided = 0 AND e.voided = 0   "
  //            + "              AND e.encounter_type = ${35}) visit3 ON visit3.patient_id =
  // p.patient_id   "
  //            + "  WHERE p.voided = 0   "
  //            + "   AND   e.voided = 0     "
  //            + "          AND e.encounter_type = ${35}   "
  ...
  //   "
  //            + "          AND DATE(visit3.terceira) <= DATE_ADD(visit2.segunda, INTERVAL 33
  // DAY);";
  //
  //    StringSubstitutor stringSubstitutor = new StringSubstitutor(map);
  //    sqlCohortDefinition.setQuery(stringSubstitutor.replace(query));
