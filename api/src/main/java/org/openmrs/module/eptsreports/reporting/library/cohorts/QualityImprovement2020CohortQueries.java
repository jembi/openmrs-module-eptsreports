...
public class QualityImprovement2020CohortQueries {
  ...
  public CohortDefinition getMQC13P3NUM_G() {
    ...
    String query =
        "SELECT p.patient_id "
            + "FROM   (SELECT art_tbl.patient_id "
            + "        FROM   (SELECT inicio.patient_id, "
            ...
            + "               ON cv.patient_id = p.patient_id "
            + "       INNER JOIN encounter e "
            + "               ON p.patient_id = e.patient_id "
            + "       INNER JOIN obs o "
            + "               ON e.encounter_id = o.encounter_id "
            + "WHERE  e.encounter_datetime BETWEEN Date_add(cv.cv_encounter, INTERVAL 6 month) "
            + "                                    AND "
            + "                                           Date_add(cv.cv_encounter, "
            + "                                           INTERVAL 9 month)";
