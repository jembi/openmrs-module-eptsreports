...
public class TxCurrCohortQueries {
  ...
  public CohortDefinition getPatientsWithLessThan3MonthlyTypeOfDispensation() {
    ...
    + "         AND IF(en.patient_id IN (SELECT        "
    + "                 ex.patient_id AS patient_id       "
    + "             FROM       "
    + "                 (SELECT        "
    + "                     e.patient_id AS patient_id, e.encounter_datetime       "
    + "                 FROM       "
    + "                     encounter e       "
    + "                 INNER JOIN obs o ON e.encounter_id = o.encounter_id       "
    + "                 WHERE       "
    ...
    + "                         AND e.encounter_datetime <= '2020-01-20'       "
    + "                         AND e.location_id = 271       "
    + "                 GROUP BY e.patient_id) AS b       "
    + "                     INNER JOIN       "
    + "                 encounter ex ON ex.patient_id = b.patient_id       "
    + "                     AND ex.encounter_datetime = b.encounter_datetime       "
    + "                     INNER JOIN       "
    + "                 obs o ON ex.encounter_id = o.encounter_id       "
    + "             WHERE       "
    + "                 ex.voided = 0 AND o.voided = 0       "
    + "                     AND ex.patient_id = en.patient_id       "
    + "                     AND ex.encounter_type = 18       "
    + "                     AND o.concept_id = 5096       "
    + "                     AND ex.encounter_datetime = last_encounter.encounter_date       "
    + "                     AND b.encounter_datetime = last_encounter.encounter_date       "
    + "                     AND ex.encounter_datetime <= '2020-01-20'       "
    + "                     AND ex.location_id = 271       "
    + "             GROUP BY ex.patient_id),       "
    + "         18,       "
    + "         en.encounter_type) = en.encounter_type       "
  ...
  public SqlCohortDefinition getPatientsWithQuarterlyTypeOfDispensation() {
    ...
    String sqlQuery =
       ...
       + "(SELECT  ex.patient_id as patient_id           "
       + "FROM           "
       ...
       + "OR (o.concept_id = 23730 AND o.value_coded in (1256,1257)))           "
       + "AND e.encounter_datetime <= '2020-01-20'           "
       + "AND e.location_id = 271           "
       + "GROUP BY e.patient_id) as b           "
       + "INNER JOIN encounter ex  on ex.patient_id = b.patient_id AND ex.encounter_datetime = b.encounter_datetime           "
 ...
 /**
   ...
   */
  public CohortDefinition
      monthlyDispensationComposition() {
    CompositionCohortDefinition cd = new CompositionCohortDefinition();
  ...
  public CohortDefinition
      semiAnnualDispensationComposition() {
    CompositionCohortDefinition cd = new CompositionCohortDefinition();
    cd.setName("Get patients with Quarterly ARV Dispensation");
    cd.addParameter(new Parameter("onOrAfter", "Start Date", Date.class));
    cd.addParameter(new Parameter("onOrBefore", "End Date", Date.class));
