  ...
  public SqlCohortDefinition getPatientsWithQuarterlyTypeOfDispensation() {
    ...
    String sqlQuery =
        "SELECT p.patient_id "
            ...
            + "    INNER JOIN ( "
            + "        SELECT "
            + "            last_encounter.patient_id, MAX(recent_date) encounter_date,  MAX(dispensation_date) dispensation_date "
            + "        FROM (  "
            + "                SELECT e.patient_id, MAX(recent_date) recent_date, MAX(o.value_datetime) AS dispensation_date "
            + "                FROM encounter e "
            + "                    INNER JOIN obs o ON o.encounter_id = e.encounter_id "
            + "                  INNER JOIN ( "
            ...
            + "                GROUP BY e.patient_id "
            + "                UNION "
            + "                SELECT e.patient_id, MAX(encounter_datetime) recent_date, MAX(e.encounter_datetime) dispensation_date "
            + "                FROM encounter e "
            + "                    INNER JOIN obs o ON e.encounter_id = o.encounter_id "
            + "                WHERE "
            + "                    e.encounter_type = ${6} "
