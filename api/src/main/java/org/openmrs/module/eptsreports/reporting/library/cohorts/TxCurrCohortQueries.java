  ...
  public CohortDefinition getPatientsWithLessThan3MonthlyTypeOfDispensation() {
    ...
    String sqlQuery =
        "     SELECT       "
            ...
            + " FROM       "
            + "     (SELECT        "
            + "         patient_id, MAX(encounter_date) AS encounter_date       "
            + "     FROM       "
            + "         (SELECT        "
            ...
            + "AND timestampdiff(day,DATE(last_encounter.encounter_date),       "
  ...
  public SqlCohortDefinition getPatientsWithQuarterlyTypeOfDispensation() {
    ...
    String sqlQuery =
        "SELECT en.patient_id           "
            ...
            + "    GROUP BY p.patient_id) as  true_last            "
            + "    GROUP BY patient_id) as last_encounter             "
            + "INNER JOIN  encounter en  ON en.patient_id = last_encounter.patient_id AND DATE(en.encounter_datetime) = DATE(last_encounter.encounter_date)           "
            ...
            + "AND ob.value_datetime IS NOT NULL            "
            + "AND timestampdiff(day,DATE(last_encounter.encounter_date),ob.value_datetime) BETWEEN 83 AND 173)           "
            ...
            + "AND ob.obs_id NOT IN           "
            + "(SELECT same_day.obs_id FROM          "
            + "(SELECT b.patient_id,b.encounter_id,b.obs_id,b.concept_id,b.encounter_datetime          "
            + "FROM          "
            + "(SELECT  e.patient_id, e.encounter_datetime, e.encounter_id, o.obs_id,o.concept_id          "
            + "FROM encounter e           "
            + "INNER JOIN obs o on e.encounter_id = o.encounter_id           "
            + "WHERE e.voided = 0            "
            + "AND o.voided = 0           "
            ...
            + "AND o.voided = 0           "
            + "AND e.encounter_type = ${6}           "
            + "AND (o.concept_id = ${23730} OR o.value_coded = ${23720})          "
            + "AND e.encounter_datetime <= :onOrBefore           "
            + "AND e.location_id = :location) as b           "
            + " LEFT JOIN encounter ex on ex.patient_id = b.patient_id AND DATE(ex.encounter_datetime) = DATE(b.encounter_datetime)           "
            + "WHERE ex.voided = 0           "
            ...
            + "AND same_day.encounter_id NOT IN         "
            + " (SELECT e.encounter_id from encounter e          "
            + "INNER JOIN obs o ON e.encounter_id = o.encounter_id          "
            + "WHERE e.encounter_type = ${18} AND o.concept_id = ${5096}          "
  ...
  public SqlCohortDefinition getPatientsWithSemiAnnualTypeOfDispensation() {
    ...
    String sqlQuery =
        "SELECT en.patient_id          "
            ...
            + " AND ((en.encounter_type = ${18} AND ob.concept_id = ${5096}           "
            + " AND ob.value_datetime IS NOT NULL           "
            + " AND timestampdiff(day,DATE(last_encounter.encounter_date),ob.value_datetime) > 173)          "
            + " OR (en.encounter_type = ${6} AND           "
            ...
            + "AND o.voided = 0           "
            + "AND e.encounter_type = ${6}           "
            + "AND (o.concept_id = ${23888} OR o.value_coded = ${23888})         "
            + "AND e.encounter_datetime <= :onOrBefore           "
            + "AND e.location_id = :location) as b           "
            + "LEFT JOIN encounter ex on ex.patient_id = b.patient_id AND DATE(ex.encounter_datetime) = DATE(b.encounter_datetime)           "
            + "WHERE ex.voided = 0           "
