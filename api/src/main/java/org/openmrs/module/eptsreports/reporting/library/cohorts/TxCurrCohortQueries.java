...
public class TxCurrCohortQueries {
	...
	public CohortDefinition getTxCurrCompositionCohort(String cohortName, boolean currentSpec) {
		...
		txCurrComposition.getSearches().put("111",
				EptsReportUtils.map(inARTProgramAtEndDate, "onOrBefore=${onOrBefore},locations=${location}"));
		txCurrComposition.getSearches().put("1", EptsReportUtils.map(getPatientWithSTARTDRUGSObsBeforeOrOnEndDate(),
				"onOrBefore=${onOrBefore},location=${location}"));

		txCurrComposition.getSearches().put("2",
				EptsReportUtils.map(hivCohortQueries.getPatientWithHistoricalDrugStartDateObsBeforeOrOnEndDate(),
						"onOrBefore=${onOrBefore},location=${location}"));

		txCurrComposition.getSearches().put("3", EptsReportUtils.map(
				getPatientEnrolledInArtProgramByEndReportingPeriod(), "onOrBefore=${onOrBefore},location=${location}"));

		txCurrComposition.getSearches().put("4",
				EptsReportUtils.map(getPatientWithFirstDrugPickupEncounterBeforeOrOnEndDate(),
						"onOrBefore=${onOrBefore},location=${location}"));

		txCurrComposition.getSearches().put("555", EptsReportUtils.map(getPatientsWhoLeftARTProgramBeforeOrOnEndDate(),
				"onOrBefore=${onOrBefore},location=${location}"));
		txCurrComposition.getSearches().put("5",
				EptsReportUtils.map(getPatientsWhoHavePickedUpDrugsMasterCardByEndReporingPeriod(),
						"onOrBefore=${onOrBefore},location=${location}"));
		// section 2
		txCurrComposition.getSearches().put("666", EptsReportUtils.map(getPatientsThatMissedNexPickup(),
		...
		txCurrComposition.getSearches().put("6",
				EptsReportUtils.map(getPatientsDeadTransferredOutSuspensionsInProgramStateByReportingEndDate(),
						"onOrBefore=${onOrBefore},location=${location}"));
		txCurrComposition.getSearches().put("777", EptsReportUtils.map(getPatientsThatDidNotMissNextConsultation(),
				String.format("onOrBefore=${onOrBefore},location=${location},abandonmentDays=%s", abandonmentDays)));
		txCurrComposition.getSearches().put("7",
				EptsReportUtils.map(getDeadPatientsInDemographiscByReportingEndDate(), "onOrBefore=${onOrBefore}"));
		txCurrComposition.getSearches().put("888", EptsReportUtils.map(
				getPatientsReportedAsAbandonmentButStillInPeriod(),
				String.format("onOrBefore=${onOrBefore},location=${location},abandonmentDays=%s", abandonmentDays)));
		txCurrComposition.getSearches().put("8",
				EptsReportUtils.map(getPatientDeathRegisteredInLastHomeVisitCardByReportingEndDate(),
						"onOrBefore=${onOrBefore},location=${location}"));
		txCurrComposition.getSearches().put("9",
				EptsReportUtils.map(getDeadPatientsInFichaResumeAndClinicaOfMasterCardByReportEndDate(),
						"onOrBefore=${onOrBefore},location=${location}"));
		txCurrComposition.getSearches().put("10",
				EptsReportUtils.map(getTransferredOutPatientsInFichaResumeAndClinicaOfMasterCardByReportEndDate(),
						"onOrBefore=${onOrBefore},location=${location}"));
		txCurrComposition.getSearches().put("11",
				EptsReportUtils.map(getPatientSuspendedInFichaResumeAndClinicaOfMasterCardByReportEndDate(),
						"onOrBefore=${onOrBefore},location=${location}"));
		txCurrComposition.getSearches().put("12",
				EptsReportUtils.map(getPatientWhoAfterMostRecentDateHaveDrugPickupOrConsultationComposition(),
						"onOrBefore=${onOrBefore},location=${location}"));
		// section 3
		txCurrComposition.getSearches().put("13",
				EptsReportUtils.map(getPatientHavingLastScheduledDrugPickupDateDaysBeforeEndDate(28),
						"onOrBefore=${onOrBefore},location=${location}"));
		txCurrComposition.getSearches().put("14",
				EptsReportUtils.map(getPatientWithoutScheduledDrugPickupDateMasterCardAmdArtPickup(),
						"onOrBefore=${onOrBefore},location=${location}"));

		txCurrComposition.getSearches().put("15", EptsReportUtils.map(getPatientsTransferedOutInLastHomeVisitCard(),
				"onOrBefore=${onOrBefore},location=${location}"));

		String compositionString;
		if (currentSpec) {
			compositionString = "(1 OR 2 OR 3 OR 4 OR 5) AND NOT ((6 OR 7 OR 8 OR 9 OR 10 OR 11 OR 15) AND NOT 12) AND NOT (13 OR 14)";
		} else {
	...
	/**
	 * 1. Cohort of patients registered as START DRUGS (answer to question 1255 =
	 * ARV PLAN is 1256 = START DRUGS) in the first drug pickup (encounter type
	...
	public CohortDefinition getPatientWithSTARTDRUGSObsBeforeOrOnEndDate() {
		...
		patientWithSTARTDRUGSObs.setQuery(TXCurrQueries.getPatientWithSTARTDRUGSObsBeforeOrOnEndDate(
	...
	/**
	 * 3.All patients enrolled in ART Program by end of reporting period. (3) Table:
	 * patient_program Criterias: program_id=2, patient_state_id=29 and
	 * date_enrolled <= endDate
	 */
	...
	public CohortDefinition getPatientEnrolledInArtProgramByEndReportingPeriod() {
		...
		definition.setQuery(TXCurrQueries.getPatientEnrolledInArtProgramByEndReportingPeriod(
	 ...
	 * 4.
	 *
	 * @return Cohort of patients with first drug pickup (encounter type 18=S.TARV:
	...
	public CohortDefinition getPatientWithFirstDrugPickupEncounterBeforeOrOnEndDate() {
		...
		patientWithFirstDrugPickupEncounter
				.setQuery(TXCurrQueries.getPatientWithFirstDrugPickupEncounterBeforeOrOnEndDate(
						hivMetadata.getARVPharmaciaEncounterType().getEncounterTypeId()));
	...
	/**
	 * 5. All patients who have picked up drugs (Recepção Levantou ARV) – Master
	 * Card by end of reporting period Encounter Type Ids = 52 The earliest “Data de
	 * Levantamento” (Concept Id 23866 value_datetime) <= endDate
	 *
	 * @return
	 */
	...
	public CohortDefinition getPatientsWhoHavePickedUpDrugsMasterCardByEndReporingPeriod() {
		...
		definition
				.setQuery(String.format(query, hivMetadata.getMasterCardDrugPickupEncounterType().getEncounterTypeId(),
	...
	/**
	 * 6. All deaths, Transferred-out and Suspensions registered in Patient Program
	 * State by reporting end date Patient_program.program_id =2 = SERVICO
	 * TARV-TRATAMENTO and Patient_State.state = 10 (Died) or Patient_State.state =
	 * 7 (Transferred-out) or Patient_State.state = 8 (Suspended) and
	 * Patient_State.start_date <= endDate Patient_state.end_date is null
	 */
	@DocumentedDefinition(value = "patientsDeadTransferredOutSuspensionsInProgramStateByReportingEndDate")
	public CohortDefinition getPatientsDeadTransferredOutSuspensionsInProgramStateByReportingEndDate() {
		...
		definition.setQuery(TXCurrQueries.getPatientsDeadTransferredOutSuspensionsInProgramStateByReportingEndDate(
				hivMetadata.getARTProgram().getProgramId(),
	...
	/**
	 * 7. All deaths registered in Patient Demographics by reporting end date
	 * Person.Dead=1 and death_date <= :endDate
	 */
	...
	public CohortDefinition getDeadPatientsInDemographiscByReportingEndDate() {
		...
		definition.setQuery(TXCurrQueries.getDeadPatientsInFichaResumeAndClinicaOfMasterCardByReportEndDate());
		definition.addParameter(new Parameter("onOrBefore", "onOrBefore", Date.class));
	...
	/**
	 * 8. All deaths registered in Last Home Visit Card by reporting end date Last
	 * Home Visit Card (Encounter Type 21, 36, 37) Patient not found (Concept ID
	 * 2003) = NO (Concept ID 1066) Reason of Not Finding (Concept ID 2031) = Died
	 * (COncpet Id 1383) Last Encounter_datetime <= endDate
	 *
	 * @return
	 */
	...
	public CohortDefinition getPatientDeathRegisteredInLastHomeVisitCardByReportingEndDate() {
		...
		definition.setQuery(TXCurrQueries.getPatientDeathRegisteredInLastHomeVisitCardByReportingEndDate(
	...
	/**
	 * 9. All deaths registered in Ficha Resumo and Ficha Clinica of Master Card by
	 * reporting end date Encounter Type ID= 6 or 53 Estado de Permanencia (Concept
	 * Id 6272) = Dead (Concept ID 1366) Encounter_datetime <= endDate
	 */
	...
	/**
	 * 10. All tranferred-outs registered in Ficha Resumo and Ficha Clinica of
	 * Master Card by reporting end date Encounter Type ID= 6 or 53 Estado de
	 * Permanencia (Concept Id 6272) = Transferred-out (Concept ID 1706)
	 * Encounter_datetime <= endDate
	 */
	@DocumentedDefinition(value = "transferredOutPatientsInFichaResumeAndClinicaOfMasterCardByReportEndDate")
	public CohortDefinition getTransferredOutPatientsInFichaResumeAndClinicaOfMasterCardByReportEndDate() {
		SqlCohortDefinition definition = new SqlCohortDefinition();
		definition.setName("transferredOutPatientsInFichaResumeAndClinicaOfMasterCardByReportEndDate");

		definition.setQuery(TXCurrQueries.getTransferredOutPatientsInFichaResumeAndClinicaOfMasterCardByReportEndDate(
				hivMetadata.getAdultoSeguimentoEncounterType().getEncounterTypeId(),
				hivMetadata.getStateOfStayOfArtPatient().getConceptId(),
				hivMetadata.getMasterCardEncounterType().getEncounterTypeId(),
				hivMetadata.getStateOfStayPriorArtPatientConcept().getConceptId(),
	...
	/**
	 * 11. All suspensions registered in Ficha Resumo and Ficha Clinica of Master
	 * Card by reporting end date Encounter Type ID= 6 or 53 Estado de Permanencia
	 * (Concept Id 6272) = Suspended (Concept ID 1709) Encounter_datetime <= endDate
	 */
	@DocumentedDefinition(value = "patientSuspendedInFichaResumeAndClinicaOfMasterCardByReportEndDate")
	public CohortDefinition getPatientSuspendedInFichaResumeAndClinicaOfMasterCardByReportEndDate() {
		...
		definition.setQuery(TXCurrQueries.getPatientSuspendedInFichaResumeAndClinicaOfMasterCardByReportEndDate(
				hivMetadata.getAdultoSeguimentoEncounterType().getEncounterTypeId(),
				hivMetadata.getStateOfStayOfArtPatient().getConceptId(),
				hivMetadata.getMasterCardEncounterType().getEncounterTypeId(),
				hivMetadata.getStateOfStayPriorArtPatientConcept().getConceptId(),
	...
	/**
	 * 13..All patients having the most recent date between last scheduled drug
	 * pickup date (Fila) or last scheduled consultation date (Ficha Seguimento or
	 * Ficha Clínica) or 30 days after last ART pickup date (Recepção – Levantou
	 * ARV) and adding {@code numDays} days and this date being less than reporting
	 * end Date. (For more clarifications refer to scenario Table 1)
	 *
	 * @param numDays
	 */
	@DocumentedDefinition(value = "patientHavingLastScheduledDrugPickupDate")
	public CohortDefinition getPatientHavingLastScheduledDrugPickupDateDaysBeforeEndDate(int numDays) {
	...
	/**
	 * 14. All patients who do not have the next scheduled drug pick up date (Fila)
	 * and next scheduled consultation date (Ficha de Seguimento or Ficha Clinica –
	 * Master Card) and ART Pickup date (Recepção – Levantou ARV).
	 *
	 * @return
	 */
	...
	public CohortDefinition getPatientWithoutScheduledDrugPickupDateMasterCardAmdArtPickup() {
		...
		definition.setQuery(TXCurrQueries.getPatientWithoutScheduledDrugPickupDateMasterCardAmdArtPickup(
				hivMetadata.getAdultoSeguimentoEncounterType().getEncounterTypeId(),
				hivMetadata.getPediatriaSeguimentoEncounterType().getEncounterTypeId(),
				hivMetadata.getARVPharmaciaEncounterType().getEncounterTypeId(),
	 ...
	 * 555
	 *
	 * @return Cohort of patients who left ART program before or on end date(4).
	 *         Includes: dead, transferred to, stopped and abandoned (patient state
	 *         10, 7, 8 or 9)
	 */
	...
	public CohortDefinition getPatientsWhoLeftARTProgramBeforeOrOnEndDate() {
		...
		String abandonStates = StringUtils.join(Arrays.asList(
				hivMetadata.getTransferredOutToAnotherHealthFacilityWorkflowState().getProgramWorkflowStateId(),
				hivMetadata.getSuspendedTreatmentWorkflowState().getProgramWorkflowStateId(),
		...
		leftARTProgramBeforeOrOnEndDate.setQuery(
				String.format(leftARTProgramQueryString, hivMetadata.getARTProgram().getProgramId(), abandonStates));
	 ...
	 * 666
	 *
	 * @return Cohort of patients that from the date scheduled for next drug pickup
	 *         (concept 5096=RETURN VISIT DATE FOR ARV DRUG) until end date have
	 *         completed 28 days and have not returned
	 */
	 ...
	 * 777
	 *
	 * @return Cohort of patients that from the date scheduled for next follow up
	 *         consultation (concept 1410=RETURN VISIT DATE) until the end date have
	 *         not completed 28 days
	 */
	...
	private CohortDefinition getPatientsThatDidNotMissNextConsultation() {
		...
		String query = "SELECT patient_id FROM " + "(SELECT p.patient_id,max(encounter_datetime) encounter_datetime "
				...
				+ "AND o.location_id=:location AND DATEDIFF(:onOrBefore,o.value_datetime)<:abandonmentDays";
		definition.setQuery(String.format(query, hivMetadata.getAdultoSeguimentoEncounterType().getEncounterTypeId(),
				hivMetadata.getPediatriaSeguimentoEncounterType().getEncounterTypeId(),
				hivMetadata.getReturnVisitDateConcept().getConceptId()));
		definition.addParameter(new Parameter("onOrBefore", "onOrBefore", Date.class));
		definition.addParameter(new Parameter("location", "location", Location.class));
	 ...
	 * 888
	 *
	 * @return Cohort of patients that were registered as abandonment (program
	 *         workflow state is 9=ABANDONED) but from the date scheduled for next
	 *         drug pick up (concept 5096=RETURN VISIT DATE FOR ARV DRUG) until the
	 *         end date have not completed 28 days
	 */
	...
	private CohortDefinition getPatientsReportedAsAbandonmentButStillInPeriod() {
		...
		definition.setQuery(String.format(query, hivMetadata.getARTProgram().getProgramId(),
				hivMetadata.getAbandonedWorkflowState().getProgramWorkflowStateId(),
				hivMetadata.getARVPharmaciaEncounterType().getEncounterTypeId(),
				hivMetadata.getReturnVisitDateForArvDrugConcept().getConceptId()));
		definition.addParameter(new Parameter("onOrBefore", "onOrBefore", Date.class));
	...
	public CohortDefinition getPatientWhoAfterMostRecentDateHaveDrusPickupOrConsultation() {
		...
		definition.setQuery(TXCurrQueries.getPatientWhoAfterMostRecentDateHaveDrusPickupOrConsultation(
				hivMetadata.getAdultoSeguimentoEncounterType().getEncounterTypeId(),
				hivMetadata.getPediatriaSeguimentoEncounterType().getEncounterTypeId(),
				hivMetadata.getARVPharmaciaEncounterType().getEncounterTypeId(),
				hivMetadata.getMasterCardDrugPickupEncounterType().getEncounterTypeId(),
	...
	/**
	 * 12. Except all patients who after the most recent date from 2.1 to 2.6, have
	 * a drugs pick up or consultation: Encounter Type ID= 6, 9, 18 and
	 * encounter_datetime> the most recent date or Encounter Type ID = 52 and “Data
	 * de Levantamento” (Concept Id 23866 value_datetime) > the most recent date.
	 */
	@DocumentedDefinition(value = "patientWhoAfterMostRecentDateHaveDrusPickupOrConsultationComposition")
	public CohortDefinition getPatientWhoAfterMostRecentDateHaveDrugPickupOrConsultationComposition() {
		...
		defintion.setQuery(TXCurrQueries.getPatientWhoAfterMostRecentDateHaveDrusPickupOrConsultationComposition(
				hivMetadata.getAdultoSeguimentoEncounterType().getEncounterTypeId(),
				hivMetadata.getPediatriaSeguimentoEncounterType().getEncounterTypeId(),
				hivMetadata.getARVPharmaciaEncounterType().getEncounterTypeId(),
				hivMetadata.getMasterCardDrugPickupEncounterType().getEncounterTypeId(),
				...
				hivMetadata.getTransferredOutConcept().getConceptId(),
				hivMetadata.getSuspendedTreatmentConcept().getConceptId(), hivMetadata.getARTProgram().getProgramId(),
	...
	@DocumentedDefinition("patients whose next ART pick-up is scheduled for >173 days after the date of their last ART drug pick-up")
	...
	@DocumentedDefinition("Patients whose next ART pick-up is scheduled for 83-173 days after the date of their last ART drug pick-up")
	...
	@DocumentedDefinition("Patients whose next ART pick-up is scheduled for <83 days after the date of their last ART drug pick-up")
	...
	@DocumentedDefinition("Patients marked as DM on Ficha Clinica Mastercard on last Tipo de Levantamento")
	public SqlCohortDefinition getPatientsWithMonthlyTypeOfDispensation() {
		...
		patientsWithMonthlyTypeOfDispensation.addParameter(new Parameter("onOrBefore", "Before Date", Date.class));
		patientsWithMonthlyTypeOfDispensation.addParameter(new Parameter("onOrAfter", "After Date", Date.class));
		patientsWithMonthlyTypeOfDispensation.addParameter(new Parameter("location", "Location", Location.class));
		return patientsWithMonthlyTypeOfDispensation;
	}

	@DocumentedDefinition("Patients marked as DT on Ficha Clinica Mastercard on last Tipo de Levantamento")
	public SqlCohortDefinition getPatientsWithQuarterlyTypeOfDispensation() {
		...
		patientsWithQuarterlyTypeOfDispensation.addParameter(new Parameter("onOrBefore", "Before Date", Date.class));
		patientsWithQuarterlyTypeOfDispensation.addParameter(new Parameter("onOrAfter", "After Date", Date.class));
		patientsWithQuarterlyTypeOfDispensation.addParameter(new Parameter("location", "Location", Location.class));
		return patientsWithQuarterlyTypeOfDispensation;
	}

	@DocumentedDefinition("Patients marked as DS on Ficha Clinica Mastercard on last Tipo de Levantamento")
	public SqlCohortDefinition getPatientsWithSemiAnnualTypeOfDispensation() {
		...
		patientsWithSemiAnnualTypeOfDispensation.addParameter(new Parameter("onOrBefore", "Before Date", Date.class));
		patientsWithSemiAnnualTypeOfDispensation.addParameter(new Parameter("onOrAfter", "After Date", Date.class));
		patientsWithSemiAnnualTypeOfDispensation.addParameter(new Parameter("location", "Location", Location.class));
		return patientsWithSemiAnnualTypeOfDispensation;
	}

	@DocumentedDefinition("Patients with last “Dispensa Trimestral (DT)” as Iniciar (I) or Manter (C)")
	...
	@DocumentedDefinition("Patients who are marked Completed for their last “Dispensa Trimestral (DT)")
	...
	private CohortDefinition getPatientsWithNextPickupBetweenDaysAfterLastPharmacyEncounter(Integer minDays,
			Integer maxDays) {
		SqlCohortDefinition cd = new SqlCohortDefinition();
		cd.addParameter(new Parameter("onOrBefore", "Before Date", Date.class));
		cd.addParameter(new Parameter("location", "location", Location.class));

		StringBuilder sb = new StringBuilder().append("select p.patient_id ").append("from patient p ").append(
				...
				.append("               where e.voided = 0 ").append("                 and o.voided = 0 ")
		...
		if (minDays != null && maxDays != null) {
			sb.append("  where timestampdiff(DAY, last_obs.encounter_datetime, last_obs.value_datetime) BETWEEN ")
					.append(minDays).append(" AND ").append(maxDays);
		} else if (minDays == null) {
			sb.append("  where timestampdiff(DAY, last_obs.encounter_datetime, last_obs.value_datetime) < ")
					.append(maxDays);
		} else {
			sb.append("  where timestampdiff(DAY, last_obs.encounter_datetime, last_obs.value_datetime) > ")
					.append(minDays);
		...
		cd.setQuery(String.format(sb.toString(), hivMetadata.getARVPharmaciaEncounterType().getEncounterTypeId(),
				hivMetadata.getReturnVisitDateForArvDrugConcept().getConceptId(),
				hivMetadata.getARVPharmaciaEncounterType().getEncounterTypeId()));
	...
	/**
	 * 15. 2.7 = 15 since version 4 . All transferred-outs registered in Last Home
	 * Visit Card
	 */
	...
	public CohortDefinition getPatientsTransferedOutInLastHomeVisitCard() {
		...
		definition.setQuery(TXCurrQueries.getPatientsTransferedOutInLastHomeVisitCard(
				hivMetadata.getBuscaActivaEncounterType().getEncounterTypeId(),
				hivMetadata.getDefaultingMotiveConcept().getConceptId(),
				hivMetadata.getTransferredOutConcept().getConceptId(),
				hivMetadata.getAutoTransferConcept().getConceptId()));
