  ...
  public SqlCohortDefinition getPatientsWithQuarterlyTypeOfDispensation() {
    ...
    String sqlQuery =
        "SELECT en.patient_id           "
            ...
            + "AND en.encounter_id NOT IN           "
            + "(SELECT same_day.encounter_id FROM           "
            + "(SELECT ex.encounter_id,b.patient_id,b.encounter_datetime,b.encounter_type, o.concept_id, ex.encounter_type as encounter           "
            + "FROM           "
            + "(SELECT  e.patient_id, e.encounter_datetime, e.encounter_id, e.encounter_type           "
            + "FROM encounter e           "
            + "INNER JOIN obs o on e.encounter_id = o.encounter_id           "
            ...
            + "            AND en.encounter_id NOT IN            "
            + "(SELECT same_day.encounter_id FROM           "
            + "(SELECT ex.encounter_id,b.patient_id,b.encounter_datetime,b.encounter_type, o.concept_id as concept, ex.encounter_type as encounter ,b.concept_id           "
            + "FROM encounter ex LEFT JOIN           "
            ...
            + "AND e.encounter_datetime <= :onOrBefore           "
            + "AND e.location_id = :location) as b           "
            + "  on ex.patient_id = b.patient_id AND ex.encounter_datetime = b.encounter_datetime           "
            + "INNER JOIN obs o on ex.encounter_id = o.encounter_id           "
            + "WHERE ex.voided = 0           "
            + "AND o.voided = 0            "
            + "AND ex.encounter_type = ${18}           "
  ...
  public SqlCohortDefinition getPatientsWithSemiAnnualTypeOfDispensation() {
    ...
    String sqlQuery =
        "SELECT en.patient_id          "
            ...
            + "AND o.voided = 0           "
            + "AND e.encounter_type = ${6}           "
            + "AND o.concept_id IN (${23739},${23888})         "
            + "AND e.encounter_datetime <= :onOrBefore           "
            + "AND e.location_id = :location) as b           "
            + "  on ex.patient_id = b.patient_id AND ex.encounter_datetime = b.encounter_datetime           "
            + "INNER JOIN obs o on ex.encounter_id = o.encounter_id           "
